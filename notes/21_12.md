...
这个题要求的是等差子序列，常规的DP定义：`dp[i]`表示以i结尾的等差数组序列的个数

但是问题在于所有以i结尾的等差序列中，公差不一定相等

所以其实我们要求的是一个集合（这个说法真好，参考了[题解](https://leetcode-cn.com/problems/arithmetic-slices-ii-subsequence/solution/gong-shui-san-xie-xiang-jie-ru-he-fen-xi-ykvk/)）

所以现在对于位置i，需要知道公差为$d_1,d_2...$并以当前位置为结尾的子序列个数

现在使用一个map用来存储不同公差的子序列个数，即key为公差，value为个数

那我们dp就不能是一个普通数组了，使用的是一个list，list中每一个element是一个map

状态转移方程，考虑对于某一个公差d和某一个位置i，有：$dp[i]_d = \sum_{j =0} ^{i - 1}(dp[j]_d + 1)$，其中$j < i$

这里面比较关键的点在于加一，他表示的是$num[j], nums[i]$这个只有两个元素的序列，而前面的$dp[j]$表示的是我们将$nums[i]$跟在$nums[j]_d$后面，构成的新的最长的子序列

也就是说，我们现在并没有考虑数组子序列长度至少为3，这个条件，这个后面考虑如何去除，先看代码
...
但这种建图的方式有点慢，所以现在考虑使用一种新的方法表示图

## 链式前向星

我们现在明确一个图中具有N个点和一共M条边

这种方式存图需要三个数组，一个数组head大小为N，一个数组edge大小为M，一个数组next大小为M

现在要知道：

* `head[i]`存储的的是以i为起点的第一条边的存储位置（这里的第一条对应于遍历的时候的最后一次需要以i为起点的边）（存储的值是某一条边）
* `edge[i]`存储的是以第i条边的终点（存储的值是某一个节点）
* `next[i]`存储的是与第i条边具有相同的起点的下一条边（存储的值是某一条边）

比如有一个图：

![](https://cdn.jsdelivr.net/gh/SunYuanI/img/img/%E9%93%BE%E5%BC%8F%E5%89%8D%E5%90%91%E6%98%9F.png)

它的边的顺序为（起点终点）：`1 2`、`2 3`、`3 4`、`1 3`、`4 1`、`1 5`、`4 5`

我们考虑边的下标从1开始

那么对应的head数组存储的数据：注意到head[5] = 0，表示没有以5为开头的边
...
通过这种方式我们可以遍历到所有以1为起点的边的终点，end就是终点

现在关键在于如何构架好这三个数组，考虑将两个节点a, b链接

首先我们要维护一个全局变量`index`，这个变量用来表示当前遍历到的边的编号

我们每次添加边的时候都要更新上面三个数组
...
更为一般的形式，我们考虑将边的下标从0开始标，此时初始化的时候要加上一行
...
class Solution {
    private int N = 501;
    //因为对于二叉树而言，一个节点最多具有三条边，所以最大具有Nx3条边
    private int M = N * 3;
    private int[] head;
    private int[] next;
    private int[] edge;
    ...
    public List<Integer> distanceK(TreeNode root, TreeNode target, int k) {
        ...
        while (!queue.isEmpty() && k > 0) {
            ...
            for (int size = queue.size(); size > 0; size--) {
                int tmp = queue.poll();
                for (int idx = head[tmp]; idx != -1; idx = next[idx]) {
                    int end = edge[idx];
                    if (!visited[end]) {
                        queue.offer(end);
                        visited[end] = true;
...
字典树爆搜，先排序，将短的字符串先进行搜索；如果当前字符串是连接词，就将其放入rst中，注意次数不要将其放入字典树中，因为当前连接词是由更短的字符串构成的，对于后面的字符串如果更短的字符串都不能使其构成连接词，那么当前字符串因为更长，更不可能使其构成连接词，并且如果加入了字典树还会构成重复搜索

关于搜索的过程，如果搜索的时候遇到了叶子（即字典树中某节点的isEnd为true），此时可以进行下一层搜索，如果下一层搜索到了，直接返回true；如果搜索的过程中子节点为空，直接放回false就好；递归的终点是搜索的index和s的长度相等
...
class Solution {
    ...
    private boolean dfs(int index, String s) {
        ...
        while (index < s.length()) {
            ...
            if (per.children[child] == null) return false;
            per = per.children[child];
            index++;
            if (per.isEnd && dfs(index, s)) return true;
        }
        return false;
    }
}
...
当然，直觉上可以通过记忆化搜索优化爆搜的时间复杂度，在[题解](https://leetcode-cn.com/problems/concatenated-words/solution/lian-jie-ci-by-leetcode-solution-mj4d/)中使用了一个visited数组，如果当前节点的isEnd为true，就让其visited[index]为true，这样如果后来回溯到当前的index的时候可以提前结束搜索。
...
class Solution {
    ...
    private boolean dfs(int index, String s, boolean[] visited) {
        if (index == s.length()) return true;
        ...
        while (index < s.length()) {
            ...
            per = per.children[child];
            index++;
            if (per.isEnd && dfs(index, s, visited)){
                return true;   
            }
        }
        return false;
