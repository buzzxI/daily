...
一个比较简单的想法是，从前向后，能填`a`我就填`a`，最后剩下的全是`z`这样字典序肯定最小：

> 当前位置能不能填`a`取决于除去这个位置外，剩下的后面的位置全部填`z`是否可以满足`k`的条件

```java
class Solution {
	private static final char pre = 'a'- 1;
	private static final int MAX = 26;
	public String getSmallestString(int n, int k) {
		char[] chars = new char[n];
		int i = 0;
		while (n > 1) {
			int per = Math.max(1, k - (n - 1) * 26);
			chars[i++] = (char)(per + pre);
			n--;
			k -= per;
		}
		chars[i] = (char)(k + pre);
...
一个更好的想法是，我先假设全部填`a`，然后从后向前修改，如果k还有剩下，我就填`z`
...
为什么后一种想法，更好，因为后一种还可以求出，在`n,k`限制下，可以获得的字典序的最大的字符串
