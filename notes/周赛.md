...
# [AcWing 154](https://www.acwing.com/activity/content/3892/)

## [5576. 区间之内](https://www.acwing.com/problem/content/5579/)

<div style="text-align:center;">
	<a href="https://www.acwing.com/problem/content/5579/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/05/07/10:00:28:5576.png" />
	</a>
</div>
...
## [5577. 有效图](https://www.acwing.com/problem/content/5580/)

<div style="text-align:center;">
	<a href="https://www.acwing.com/problem/content/5580/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/05/07/10:02:39:5577.png" />
	</a>
</div>

如果实际画一下图, 从 4 个点的有效图开始就能看到, 每个连通块内节点都应该是全联通的 -> 完全图/团; 当尝试将第五个点加入到有效图中时, 新加入的点必须和之前的 4 个点分别建立一条边

>   具体的证明可以采用数学归纳法证明, 假设 n 个点的有效图是一个完全图, 那么对于一个 n + 1 个节点的有效图, 新节点 x, 必然和 n 个节点中的某个节点 y 相连
>
>   而原来的 n 个节点构成了完全图, 因此节点 y 具有 n 个邻居节点 (包括 x), 根据有效图的定义, 新节点 x 也必须有 n 个邻居节点

所以本题可以简化为, 判断每个连通块是不是一个完全图, 具体的可以直接判断每个节点的度是否为连通块中节点个数 - 1 即可
...
class Main{
    ...
    static StreamTokenizer in = new StreamTokenizer(ins);
    static PrintWriter out = new PrintWriter(new OutputStreamWriter(System.out));
    // p 维护父节点, w 维护当前节点下的节点个数 (只有根节点有效)
    static int[] p;
    static int[] w;
...
## [5578. 切蛋糕](https://www.acwing.com/problem/content/5581/)

<div style="text-align:center;">
	<a href="https://www.acwing.com/problem/content/5581/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/05/07/10:13:34:5578.png" />
	</a>
</div>

因为都是矩形切割, 只要能知道宽和高就可以求解面积了, 注意到垂直方向的切割不会影响每个矩形的高度, 而仅影响矩形的宽, 而水平方向的切割不会影响矩形的宽, 而仅影响高度

因此本题中两种切割是相互独立的, 这意味着可以分别维护当前最大的宽和高, 二者的乘积就是最大的矩形

因为每次切割会删除一条边, 然后添加两个短边, 因此不能直接使用堆进行维护, 这里使用一个 TreeMap 维护当前的最长边, 其中各种长度的边可能存在多个 (不然就使用 TreeSet 了)

其次需要记录每次切割的位置, 因此还需要知道当前切割的位置所在的边, 还使用了一个 TreeSet 表示当前已经使用的点, 相邻的两个点承载的边被加入 TreeMap, 而点本身被加入到 TreeSet 中
...
public class Main {
    ...
    static TreeSet<Integer> ws = new TreeSet<>();
    static TreeMap<Integer, Integer> wlen = new TreeMap<>();
    static TreeSet<Integer> hs = new TreeSet<>();
    static TreeMap<Integer, Integer> hlen = new TreeMap<>();
    public static void main(String[] args) throws IOException {
        int w, h, n;
        in.nextToken();
        w = (int)(in.nval);
        ...
        for (int i = 0; i < n; i ++) {
            ...
            if (ss[0].equals("Y")) {
                Integer floor = hs.floor(j);
                Integer ceiling = hs.ceiling(j);
                int len = ceiling - floor;
                if (hlen.get(len) > 1) {
                    hlen.put(len, hlen.get(len) - 1);
                } else hlen.remove(len);
                hlen.put(ceiling - j, hlen.getOrDefault(ceiling - j, 0) + 1);
                hlen.put(j - floor, hlen.getOrDefault(j - floor, 0) + 1);
                hs.add(j);
            } else {
                Integer floor = ws.floor(j);
                Integer ceiling = ws.ceiling(j);
                int len = ceiling - floor;
                if (wlen.get(len) > 1) {
                    wlen.put(len, wlen.get(len) - 1);
                } else wlen.remove(len);
                wlen.put(ceiling - j, wlen.getOrDefault(ceiling - j, 0) + 1);
                wlen.put(j - floor, wlen.getOrDefault(j - floor, 0) + 1);
                ws.add(j);
            }
            int hmax = hlen.floorKey(Integer.MAX_VALUE);
            int wmax = wlen.floorKey(Integer.MAX_VALUE);
            out.println((long)hmax * (long)wmax);
        }
        out.close();
    }
}
...
# [第 130 场双周赛](https://leetcode.cn/contest/biweekly-contest-130/)

## [100299. 判断矩阵是否满足条件](https://leetcode.cn/contest/biweekly-contest-130/problems/check-if-grid-satisfies-conditions/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/biweekly-contest-130/problems/check-if-grid-satisfies-conditions/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/05/12/19:08:42:100299.png" />
	</a>
</div>
...
## [100302. 正方形中的最多点数](https://leetcode.cn/contest/biweekly-contest-130/problems/maximum-points-inside-the-square/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/biweekly-contest-130/problems/maximum-points-inside-the-square/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/05/12/19:10:06:100302.png" />
	</a>
</div>

比赛的时候没想那么多, 后来才知道这题更一般的做法其实是二分, 二分半径, 最后再计算矩阵中的点数

```java
class Solution {
    public int maxPointsInsideSquare(int[][] points, String s) {
        List<List<Integer>> f = new ArrayList<>(26);
        for (int i = 0; i < 26; i ++) f.add(new ArrayList<>());
        for (int i = 0; i < s.length(); i ++) {
            int j = s.charAt(i) - 'a';
            f.get(j).add(Math.max(Math.abs(points[i][0]), Math.abs(points[i][1])));
        }
    ...
    private boolean check(List<List<Integer>> f, int r) {
        for (int i = 0; i < f.size(); i ++) {
            boolean flag = false;
            for (Integer d : f.get(i)) {
                if (d <= r) {
    ...
    private int get(List<List<Integer>> f, int r) {
        int rst = 0;
        for (int i = 0; i < f.size(); i ++) {
            boolean flag = false;
            for (Integer d : f.get(i)) {
...
比赛的时候想的是, 因为不能重复, 因此矩形内只能放一个同种类型的点, 最多只有 26 个点, 记录下每种点距离原点最近和次近点的 "半径", 最终的半径一定比所有节点的次近点的半径最小的那个稍微小一点点 -> 保证每个点都不重复
...
class Solution {
    ...
    public int maxPointsInsideSquare(int[][] points, String s) {
        ...
        for (int i = 0; i < s.length(); i ++) {
            ...
            if (freq[j][0] == -1) {
                freq[j][0] = max;
            } else if (max < freq[j][0]) {
                freq[j][1] = freq[j][0];
                freq[j][0] = max;
            } else if (max >= freq[j][0]) {
                if (freq[j][1] == -1 || max <= freq[j][1]) {
                    freq[j][1] = max;
                } 
            }
        }
        ...
        int min = INF;
        for (int i = 0; i < 26; i ++) {
            if (freq[i][1] == -1) continue;
            if (freq[i][1] < min) min = freq[i][1];
        }
...
## [100289. 分割字符频率相等的最少子字符串](https://leetcode.cn/contest/biweekly-contest-130/problems/minimum-substring-partition-of-equal-character-frequency/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/biweekly-contest-130/problems/minimum-substring-partition-of-equal-character-frequency/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/05/12/19:31:19:100289.png" />
	</a>
</div>

比赛的时候想的是, 每种分割的子串具有无后效性, 将字符串分为两段, 前半段不管怎么分, 都不会影响后半段的分法

定义状态 f[i] 表示从 i 开始进行分割, 可以得到的最小的平衡子字符串数量
...
class Solution {
    ...
    private int dfs(int idx) {
        ...
        for (int i = idx; i < s.length(); i ++) {
            int j = s.charAt(i) - 'a';
            freq[j] ++;
            if ((i - idx + 1) % freq[j] != 0) continue;
            if (check(freq)) {
                // 枚举当前段的大小, 只有当前段满足平衡条件时, 才进行分割
                rst = Math.min(rst, 1 + dfs(i + 1));
            }
        }
        buff[idx] = rst;
        return buff[idx];
...
T4 太难了, 就算是比赛完再写, 也不容易完成推导, 这种题比赛肯定是写不出来的

# [第 397 场周赛](https://leetcode.cn/contest/weekly-contest-397)

## [100296. 两个字符串的排列差](https://leetcode.cn/contest/weekly-contest-397/problems/permutation-difference-between-two-strings/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/weekly-contest-397/problems/permutation-difference-between-two-strings/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/05/12/19:39:34:100296.png" />
	</a>
</div>
...
## [100274. 从魔法师身上吸取的最大能量](https://leetcode.cn/contest/weekly-contest-397/problems/taking-maximum-energy-from-the-mystic-dungeon/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/weekly-contest-397/problems/taking-maximum-energy-from-the-mystic-dungeon/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/05/12/19:41:52:100274.png" />
	</a>
</div>
...
class Solution {
    ...
    public int maximumEnergy(int[] energy, int k) {
        ...
        Arrays.fill(buff, INF);
        
        this.energy = energy;
        this.k = k;
        
        for (int i = 0; i < n; i ++) {
            rst = Math.max(rst, dfs(i));
        }
        return rst;
    }
    
    private int dfs(int x) {
        if (x >= buff.length) return 0;
        if (buff[x] != INF) return buff[x];
        
        buff[x] = energy[x] + dfs(x + k);
        return buff[x];
    }
}
```

其实本题如果反着考虑的话, 还简单一些, 因为根据题目要求, 所有选法的序列结尾必须属于 nums[n - k] ~ nums[n - 1] 种的某一个数, 枚举每个数作为结尾的情况就好了
...
## [100281. 矩阵中的最大得分](https://leetcode.cn/contest/weekly-contest-397/problems/maximum-difference-score-in-a-grid/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/weekly-contest-397/problems/maximum-difference-score-in-a-grid/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/05/12/19:48:09:100281.png" />
	</a>
</div>

比赛的时候看了一眼就想着用记忆化搜索了, 直接来了发 TLE, 后续改成 dp 的写法, 看了一下时间复杂度为 $O(m\times n\times (m + n))$ 在本题的输入下, 时间复杂度来到了 $10^8$ 级别

考虑多次移动下, 有: $(x_1, y_1)$ -> $(x_1, y_2)$ -> $(x_2, y_2)$, 则整体的收益为: $f[x_1][y_2] - f[x_1][y_1] + f[x_2][y_2] - f[x_1][y_2] = f[x_2][y_2] - f[x_1][y_1]$

也就是不管最终移动了多少次, 最终用于计算得分的始终只有左上角的点和右下角的点, 为了得分最大, 显然应该计算节点左上方矩阵中值最小的点

因此本题被转化为: 枚举每个节点左上角矩阵中最小值, 并做差更新结果集

定义 f\[i][j] 为节点 (i, j) 左上角矩阵中的最小值, 使用类似前缀和数组的方式更新最小值, 一边更新一边计算
...
class Solution {
    ...
    public int maxScore(List<List<Integer>> grid) {
        ...
        int[][] pre = new int[n + 1][m + 1];
        for (int i = 0; i <= n; i ++) Arrays.fill(pre[i], INF);
        int rst = NINF;
        for (int i = 1; i <= n; i ++) {
            for (int j = 1; j <= m; j ++) {
                int cur = grid.get(i - 1).get(j - 1);
                pre[i][j] = Math.min(pre[i - 1][j], pre[i][j - 1]);
                rst = Math.max(rst, cur - pre[i][j]);
                pre[i][j] = Math.min(pre[i][j], cur);
            }
        }
        return rst;
    }
...
## [100312. 找出分数最低的排列](https://leetcode.cn/contest/weekly-contest-397/problems/find-the-minimum-cost-array-permutation/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/weekly-contest-397/problems/find-the-minimum-cost-array-permutation/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/05/12/20:00:42:100312.png" />
	</a>
</div>

本题的最佳排列没有什么规律, 需要枚举, 首先通过记忆化搜索计算最低分数, 然后根据分数推导字典序最小的排列

由于输入范围大小仅仅 14, 使用 mask 表示各个数字的使用情况, 同时由于当前位置的分数和前一个位置有关, 函数签名中还应该包含前一个位置的数字
...
class Solution {
    ...
    public int[] findPermutation(int[] nums) {
        this.nums = nums;
        this.n = nums.length;
        this.m = new int[1 << n][n];
        
        for (int i = 0; i < (1 << n); i ++) Arrays.fill(m[i], -1);
    ...
    private int dfs(int mask, int pre) {
        if (mask == (1 << n) - 1) {
            // 如果当前 mask 为掩码状态, 就直接返回, 后续没有新的数字了
            m[mask][pre] = Math.abs(pre - nums[0]);
            return m[mask][pre];
        }
        
        if (m[mask][pre] != -1) return m[mask][pre];
        ...
        for (int i = 0; i < n; i ++) {
            ...
            // 枚举下一个数字
            rst = Math.min(rst, dfs(mask | (1 << i), i) + Math.abs(pre - nums[i]));
        }
    ...
    private void print(int mask, int pre) {
        rst[e ++] = pre;
        ...
        for (int i = 0; i < n; i ++) {
            if (((mask >> i) & 1) == 1) continue;
            int ne = mask | (1 << i);
            if (m[ne][i] + Math.abs(pre - nums[i]) == m[mask][pre]) {
                print(ne, i);
                return;
            }
        }
...
>   比较容易出问题的点在于: 记忆化搜索的时候, mask 取到掩码时直接返回了, 此时还应该更新 buffer
>
>   此外为什么第一个数字一定是 0, 这个是猜的 ... 当然也可以根据第一个位置选择数字的不同多次记忆化搜索, 反正以 0 开头的, 肯定存在开销最小的序列

# [第 398 场周赛](https://leetcode.cn/contest/weekly-contest-398/)

## [100310. 特殊数组 I](https://leetcode.cn/contest/weekly-contest-398/problems/special-array-i/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/weekly-contest-398/problems/special-array-i/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/05/19/15:00:16:100310.png" />
	</a>
</div>

签到题, 直接判断当前元素和前一个元素的奇偶性就好了
...
## [100308. 特殊数组 II (Special Array II)](https://leetcode.cn/classic/problems/special-array-ii/description/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/classic/problems/special-array-ii/description/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/05/19/15:28:44:100308.png" />
	</a>
</div>

比赛的时候的做法是维护各个最长的特殊子数组, 对于 queries 中的每个询问, 通过二分的方式找到其左边界所属的子数组, 然后判断当前查询是否被子数组包裹, 如果被包裹则说明当前查询属于特殊数组
...
class Solution {
    public boolean[] isArraySpecial(int[] nums, int[][] queries) {
        ...
        for (int i = 0; i < m; i ++) {
            ...
            while (l < r) {
                int mid = l + ((r - l) >> 1);
                if (list.get(mid)[1] < queries[i][0]) l = mid + 1;
                else r = mid;
            }
            int[] tmp = list.get(l);
            rst[i] = (tmp[0] <= queries[i][0] && tmp[1] >= queries[i][1]);
        }
...
实际上本题有一个时间复杂度更低的做法: 维护一个长度为 n - 1 的数组 arr, arr[i] 表示 nums[i] 和 nums[i + 1] 的奇偶性是否相同, 如果相同, 则 arr[i] 为 1, 否则为 0

这样只要保证每个 query 查询区间内 arr[i] 均为 0 即可, 更进一步可以维护一个前缀和数组 pre, 通过前缀和数组的做差得到当前区间内是否存在 arr[i] 为 1 的元素

>   编码细节: queries 中的每个区间可以被抽象为 [from, to], arr[i] 维护的是 nums[i] 和 nums[i + 1] 的关系, 因此 [from, to] 的查询对应了区间 arr[from] 到 arr[to - 1]
>
>   prefix[i] 维护的是 0 ~ arr[i - 1] 的前缀和, 因此 arr[from] 到 arr[to - 1] 的区间和正好对应了 prefix[to] - prefix[from]
...
class Solution {
    public boolean[] isArraySpecial(int[] nums, int[][] queries) {
        ...
        for (int i = 1; i < n; i ++) {
            pre[i] = pre[i - 1];
            if (((nums[i] ^ nums[i - 1]) & 1) == 0) pre[i] ++; 
        }
        int m = queries.length;
        boolean[] rst = new boolean[m];
        for (int i = 0; i < m; i ++) {
            if (pre[queries[i][1]] != pre[queries[i][0]]) rst[i] = false;
            else rst[i] = true;
        }
        return rst;
    }
...
## [100300. 所有数对中数位不同之和](https://leetcode.cn/contest/weekly-contest-398/problems/sum-of-digit-differences-of-all-pairs/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/weekly-contest-398/problems/sum-of-digit-differences-of-all-pairs/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/05/19/15:39:34:100300.png" />
	</a>
</div>

本题比较特殊, nums 数组中的每个 num 长度都相同, 因此可以直接按位计算不同, 比赛的时候先进行预处理, 后计算不同的个数
    ...
    public long sumDigitDifferences(int[] nums) {
        ...
        for (int i = 0; i < N; i ++) {
            for (int j = 0; j < N; j ++) {
                ...
                long cur = arr[i][j];
                long re = 0;
                // 计算与当前位不同的数字个数 (当前为 a 时, 仅考虑 a + 1, a + 2 ... 的个数, 避免重复计数)
                for (int k = j + 1; k < N; k ++) re += arr[i][k];
                rst += cur * re;
            }
        }
...
而事实上可以将预处理的过程和统计的过程合并, 对于当前处理到的第 i 位, 数字为 j 时, 直接计算和当前数字不同即可
...
>   这种统计方式的好处是, 天然避免了重复统计的问题

## [100298. 到达第 K 级台阶的方案数](https://leetcode.cn/contest/weekly-contest-398/problems/find-number-of-ways-to-reach-the-k-th-stair/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/weekly-contest-398/problems/find-number-of-ways-to-reach-the-k-th-stair/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/05/19/15:47:15:100298.png" />
	</a>
</div>

>   组合数学应用题

每次 jump 过后, jump 都会自增, 对于一个长为 j 的序列, 其一定有如下格式: $2^0, 2^1, 2^2 ... 2^{j - 1}$

除了 jump 之外, 还可以通过 -1 的操作移动到 i - 1 的位置, 但在题目的设定中不能连续进行 -1 操作, 两个 -1 之间必须插入一个 jump

对于一个长为 j 的序列, 其最多插入 j + 1 个 -1, 此时序列具有如下格式: $-1, 2^0, -1, 2^1, -1, 2^2, -1 ... -1, 2^{j - 1}, -1$

注意到输入范围仅仅 $10^9$ 级别, 也就是不到 $2^{30}$ 的级别, 最多插入 31 个 -1

因此这里使用一个 map 记录所有可以构成的数字的方案数, 然后枚举序列的长度 j, 从 1 枚举到 31 (序列的最后一项为 $2^{30}$), 对于每轮枚举内部继续枚举 -1 的个数 k, 从 0 个最多到 j + 1 个,  通过组合数 $C_{j + 1}^{k}$ 计算方案个数
...
class Solution {
    ...
    static {
        ...
        // 默认情况下, 存在一种方案, 组合结果为 0
        m.put(0L, 1);
        // 在不 jump 的情况下, 可以直接 -1 一次
        m.put(-1L, 1);
        long base = 0;
       	// 每轮枚举添加一个 2^i, 一共到 2^30
        for (int i = 0; i <= N; i ++) {
            base += (1 << i);
            // re 表示可以插入 -1 的位置
            int re = 2 + i;
            for (int j = 0; j <= re; j ++) {
                int x = Math.min(j, re - j);
                long ne = base - j;
                int pre = m.getOrDefault(ne, 0);
                // 组合数计算方案个数
                pre += B[re][x];
                m.put(ne, pre);
            }
    ...
    public int waysToReachStair(int k) {
        // 最开始为 1, 因此这里需要计算的是和 1 的距离
        long gap = k - 1;
        return m.getOrDefault(gap, 0);
    }
}
```

# [第 131 场双周赛](https://leetcode.cn/contest/biweekly-contest-131/)

## [100309. 求出出现两次数字的 XOR 值 (Find the XOR of Numbers Which Appear Twice)](https://leetcode.cn/classic/problems/find-the-xor-of-numbers-which-appear-twice/description/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/classic/problems/find-the-xor-of-numbers-which-appear-twice/description/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/05/26/17:11:37:100309.png" />
	</a>
</div>
...
## [100303. 查询数组中元素的出现位置 (Find Occurrences of an Element in an Array)](https://leetcode.cn/classic/problems/find-occurrences-of-an-element-in-an-array/description/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/classic/problems/find-occurrences-of-an-element-in-an-array/description/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/05/26/17:09:35:100303.png" />
	</a>
</div>
...
class Solution {
    public int[] occurrencesOfElement(int[] nums, int[] queries, int x) {
        ...
        int[] rst = new int[m];
        for (int i = 0; i < m; i ++) {
            if (queries[i] > freq.size()) rst[i] = -1;
            else rst[i] = freq.get(queries[i] - 1);
        }
...
## [100313. 所有球里面不同颜色的数目 (Find the Number of Distinct Colors Among the Balls)](https://leetcode.cn/classic/problems/find-the-number-of-distinct-colors-among-the-balls/description/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/classic/problems/find-the-number-of-distinct-colors-among-the-balls/description/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/05/26/17:07:26:100313.png" />
	</a>
</div>
    ...
    public int[] queryResults(int limit, int[][] queries) {
        ...
        for (int i = 0; i < n; i ++) {
            if (color.containsKey(queries[i][0])) {
                int c = color.get(queries[i][0]);
                Set<Integer> set = m.get(c);
                set.remove(queries[i][0]);
                if (set.isEmpty()) m.remove(c);
            }
            
            Set<Integer> set = m.getOrDefault(queries[i][1], new HashSet<>());
            set.add(queries[i][0]);
            m.put(queries[i][1], set);
...
## [100314. 物块放置查询](https://leetcode.cn/contest/biweekly-contest-131/problems/block-placement-queries/description/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/biweekly-contest-131/problems/block-placement-queries/description/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/05/26/16:43:30:100314.png" />
	</a>
</div>

最暴力的做法: 枚举区间, 整体时间复杂度为 $O(n\times(m - n))$​, 其中 m 表示 queries 大小, n 表示插入障碍物的数量, 基本上就是个 $O(n^2)$ 级别的输入, 本题直接寄掉

因为涉及到单点更新 (插入障碍物), 和区间查询, 最大区间, 比赛的时候想到了使用线段树, 但是感觉如果区间还是使用区间折半的方式分割的话, 可能会出现线段树某个区间被障碍物分割的情况, 所以比赛的时候搞了怪东西, 按照障碍物分割区间:

```java
class Solution {
    private static final int N = (int)5e4 + 10;
    public List<Boolean> getResults(int[][] queries) {
        ...
        for (int[] q : queries) {
            if (q[0] == 1) separate(root, q[1]);
            else {
                int max = query(root, q[1]);
                rst.add(max >= q[2]);
            }
        }
    ...
    private void separate(Node node, int x) {
        if (node.left == null && node.right == null) {
            node.left = new Node();
            node.left.l = node.l;
            node.left.r = x;
            node.left.val = node.left.r - node.left.l;   
            node.right = new Node();
            node.right.l = x;
            node.right.r = node.r;
            node.right.val = node.right.r - node.right.l;
            node.val = Math.max(node.left.val, node.right.val);
            return;
        }
        
        int m = node.left.r;
        if (x <= m) separate(node.left, x);
        else separate(node.right, x);
        node.val = Math.max(node.left.val, node.right.val);
    }
    ...
    private int query(Node node, int x) {
        ...
        int m = node.left.r;
        if (x <= m) return query(node.left, x);
        else return Math.max(node.left.val, query(node.right, x));
    }
}
...
每个节点的左右边界通过 l 和 r 确定, 当插入一个障碍物时, 一定是对线段树的某个节点进行了分割, 从某些程度上来讲, 这种线段树还比正常的更好写了, 然后就爆 TLE 了, 比赛结束了之后看了一眼输入

```shell
[[1,2],[1,4],[1,6],[1,8],[1,10],[1,12],[1,14],[1,16],[1,18],[1,20],[1,22],[1,24],[1,26],[1,28],[1,30],[1,32],[1,34],[1,36],[1,38],[1,40],[1,42],[1,44],[1,46],[1,48],[1,50],[1,52],[1,54],[1,56],[1,58],[1,60],[1,62],[1,64],[1,66],[1,68],[1,70],[1,72],[1,74],[1,76]...
```

他是一个先不断分割, 然后再查询的输入, 如果使用我上面的线段树的话, 由于每个节点的左右边界是通过障碍物分割的, 因此线段树直接被退化为链表了, 每次插入等价于到链表的最后一个节点进行插入, 整体时间复杂度为 $O(n^2)$

>   这下知道为什么线段树需要使用 mid 作为左右子节点的分割边界了 ...

本题依旧使用线段树, 但是不再被节点会被障碍物分割的情况, 就把线段树看成是一个维护了区间最大值的简单线段树, 对障碍物区间的右边界赋值为区间长度, 每次查询时, 只根据已有区间查询
...
class Solution {
    ...
    public List<Boolean> getResults(int[][] queries) {
        ...
        for (int[] q : queries) {
            if (q[0] == 1) {
                // 新障碍物插入, 分割得到两个区间, 障碍物节点更新为新区间大小
                int x = q[1];
                int next = set.ceiling(x);
                int pre = set.floor(x);
                update(root, 0, N, x, x - pre);
                update(root, 0, N, next, next - x);
                set.add(x);
            } else {
                int x = q[1];
                // 仅在障碍物范围内做区间查询
                int pre = set.floor(x);
                int max = query(root, 0, N, pre);
                // 对于查询区间大于线段树保存区间的情况, 单独处理即可
                max = Math.max(max, x - pre);
                rst.add(max >= q[2]);
            }
        }
    ...
    private void update(Node node, int l, int r, int x, int v) {
        if (l == r) {
            node.val = v;
            return;
        }
        ...
        createLazy(node);
        int m = l + ((r - l) >> 1);
        if (x <= m) update(node.left, l, m, x, v);
        else update(node.right, m + 1, r, x, v);

        node.val = Math.max(node.left.val, node.right.val);
    }

    private int query(Node node, int l, int r, int x) {
        if (r <= x) return node.val;

        int m = l + ((r - l) >> 1);
        if (x <= m) return query(node.left, l, m, x);
        return Math.max(node.left.val, query(node.right, m + 1, r, x));
    }
...
## [100326. 压缩字符串 III](https://leetcode.cn/contest/weekly-contest-399/problems/string-compression-iii/)

<div style="text-align:center;">
	<a href="(https://leetcode.cn/contest/weekly-contest-399/problems/string-compression-iii/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/05/26/18:00:03:100326.png" />
	</a>
</div>
...
class Solution {
    public String compressedString(String word) {
        int n = word.length();
        char[] rst = new char[n << 1];
        int e = 0;
        
        char[] cs = word.toCharArray();
        int l = 0, r = 1;
        for (; r < n; r ++) {
            if (cs[r] == cs[r - 1]) continue;
            int cnt = r - l;
            while (cnt > 0) {
                int val = Math.min(cnt, 9);
                rst[e ++] = (char)(val + '0');
                rst[e ++] = cs[l];
                cnt -= val;
            }
        ...
        return new String(rst, 0, e);
    }
}
```

## [100321. 优质数对的总数](https://leetcode.cn/contest/weekly-contest-399/problems/find-the-number-of-good-pairs-ii/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/weekly-contest-399/problems/find-the-number-of-good-pairs-ii/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/05/26/17:33:05:100321.png" />
	</a>
</div>

> 这次周赛的 1, 3 题长一个样, 只有输入范围不一样, 这里以 3 题的输入范围为限制

最暴力的方法就是挨个数字枚举, 整体时间复杂度为 $O(n\times m)$, 本题中达到了 $10^{10}$ 级别, 直接寄

本题首先通过将乘法转化为除法, 对 nums1 进行筛选, 只有 nums1 中和 k 取模为 0 的数字才能参与到数对的统计中

其次注意到 nums2 中数字 n2 可以整除 nums1 中数字 n1 的前提是, n2 是 n1 的一个因数, 因此这里完全可以对 nums1 进行预处理, 统计一个因数数组, 维护了每个数字作为 nums1 中可以作为因数的数字个数

这样 n2 在判断可以整除数字的个数的时候, 直接查看有多少个数字以自己作为因数即可
...
## [100306. 不包含相邻元素的子序列的最大和](https://leetcode.cn/contest/weekly-contest-399/problems/maximum-sum-of-subsequence-with-non-adjacent-elements/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/weekly-contest-399/problems/maximum-sum-of-subsequence-with-non-adjacent-elements/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/05/26/17:13:55:100306.png" />
	</a>
</div>

讲道理, 不包含相邻数字的子序列最大值, dp 搜一次就能搜出来, 问题在于本题还需要进行修改, 最暴力的方法是: 每次修改后重新从修改的位置向后继续 dp 计算, 整体时间复杂度达到了 $O(n\times m)$ 本题就是 $10^9$ 级别肯定寄

本题优化的点在于, 使用递归的方式计算不包含相邻数字的最大和 -> 这样在修改后, 就可以通过线段树进行快速查询

>   如果某个问题可以通过递归的方式计算得到, 那么在原问题的基础上进行区间内的修改后再查询, 就可以通过线段树计算

对于原序列 a 分为前后两部分 $a_1$ 和 $a_2$, 显然原序列的最大值不能直接用两部分的最大值相加得到, 还需要考虑拼接处的取值, 定义状态 $a_1[1][0]$ 表示前半段序列, 在不考虑序列最后一个数字时可以得到最大和, 而状态 $a_2[0][1]$ 表示后半段序列, 在不考虑序列第一个数字时可以得到的最大和

显然当前序列的最大和 $a[1][1]$ 可以表示为 $\max(a_1[1][0]+ a_2[1][1], a_1[1][1] + a_2[0][1])$ (状态 $a[1][1]$ 表示考虑序列中第一个数字和最后一个数字在内的最大和)

进一步的状态 $a[1][0]$ 可以表示为 $\max(a_1[1][1] + a_2[0][0], a_1[1][0] + a_2[1][0])$, 注意到这里存在一个状态 $a[0][0]$ 它表示不考虑序列中第一个数字和最后一个数字的最大和

剩下的两个状态转移方程为: $a[0][1] = \max(a_1[0][0] + a_2[1][1], a_1[0][1] + a_2[0][1]); a[0][0] = \max(a_1[0][1] + a_2[0][0], a_1[0][0] + a_2[1][0])$

递归终点, 当区间不能再分时, 有 $a[0][0] = a[0][1] = a[1][0] = 0; a[1][1] = \max(0, arr[i])$

综上, 一个不包含相邻数字求解最大值的操作可以通过一个递归算法求解得到, 那么按照上面的说法, 只要能用递归, 在修改后, 可以通过线段树维护, 此时线段树节点维护的信息就是上述的四个状态
...
class Solution {
    ...
    public int maximumSumSubsequence(int[] nums, int[][] queries) {
        int n = nums.length;
        Node root = new Node();
        for (int i = 0; i < n; i ++) update(root, 0, N, i, nums[i]);
        
        int rst = 0;
        int m = queries.length;
        for (int i = 0; i < m; i ++) {
            update(root, 0, N, queries[i][0], queries[i][1]);
            // 不需要写 query 函数, root 节点已经维护了状态信息
            rst = (int)((rst + root.f[3]) % MOD);
        }
        return rst;
    }
    
    private void update(Node node, int l, int r, int x, long v) {
        if (l == r) {
            node.f[3] = Math.max(v, 0L);
            return;
        }
        ...
        node.f[0] = Math.max(node.left.f[1] + node.right.f[0], node.left.f[0] + node.right.f[2]);
        node.f[1] = Math.max(node.left.f[0] + node.right.f[3], node.left.f[1] + node.right.f[1]);
        node.f[2] = Math.max(node.left.f[3] + node.right.f[0], node.left.f[2] + node.right.f[2]);
        node.f[3] = Math.max(node.left.f[3] + node.right.f[1], node.left.f[2] + node.right.f[3]);
    }
...
# [第 400 场周赛](https://leetcode.cn/contest/weekly-contest-400)

## [100307. 候诊室中的最少椅子数](https://leetcode.cn/contest/weekly-contest-400/problems/minimum-number-of-chairs-in-a-waiting-room/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/weekly-contest-400/problems/minimum-number-of-chairs-in-a-waiting-room/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/06/02/14:14:49:100307.png" />
	</a>
</div>

直接模拟人数的变化即可, 返回值为人数的最大值
...
## [100311. 无需开会的工作日](https://leetcode.cn/contest/weekly-contest-400/problems/count-days-without-meetings/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/weekly-contest-400/problems/count-days-without-meetings/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/06/02/14:17:04:100311.png" />
	</a>
</div>

合并区间即可

> 比赛的时候没有注意到 meetings\[i][1] <= days 的条件

```java
class Solution {
    public int countDays(int days, int[][] ms) {
        Arrays.sort(ms, (m1, m2) -> {
            if (m1[0] == m2[0]) return m2[1] - m1[1];
            return m1[0] - m2[0];
        });
        int n = ms.length;
        int rst = 0;
        ...
        int r = ms[0][1];
        for (int i = 1; i < n; i ++) {
            if (ms[i][0] <= r) r = Math.max(r, ms[i][1]);
            else {
                r = Math.min(r, days);
                rst += r - l + 1;
                l = ms[i][0];
...
## [100322. 删除星号以后字典序最小的字符串](https://leetcode.cn/contest/weekly-contest-400/problems/lexicographically-minimum-string-after-removing-stars/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/weekly-contest-400/problems/lexicographically-minimum-string-after-removing-stars/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/06/02/14:20:40:100322.png" />
	</a>
</div>

根据题目规则, 每个星号会删除掉原字符串中的字典序最小的字符, 如果存在多个相同字符, 则为了使得字典序最小, 应该让小字符串中位置靠后的字符被删除
...
class Solution {
    public String clearStars(String s) {
        ...
        int n = cs.length;
        // 双端队列维护相同字符的位置
        List<Deque<Integer>> freq = new ArrayList<>(26);
        for (int i = 0; i < 26; i ++) freq.add(new ArrayDeque<>(n));
        
        for (int i = 0; i < n; i ++) {
            int j = cs[i] - 'a';
            if (j >= 0 && j < 26) freq.get(j).offerLast(i);
            else {
                for (int k = 0; k < 26; k ++) {
                    if (!freq.get(k).isEmpty()) {
                        freq.get(k).pollLast();
                        break;
                    }
                }
        ...
        for (int i = 0; i < n; i ++) {
            int j = cs[i] - 'a';
            if (j >= 0 && j < 26) {
                if (!freq.get(j).isEmpty() && freq.get(j).peekFirst() == i) {
                    rst[e ++] = cs[i];
                    freq.get(j).pollFirst();
                }
            }
        }
...
## [100315. 找到按位与最接近 K 的子数组](https://leetcode.cn/contest/weekly-contest-400/problems/find-subarray-with-bitwise-and-closest-to-k/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/weekly-contest-400/problems/find-subarray-with-bitwise-and-closest-to-k/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/06/02/15:04:42:100315.png" />
	</a>
</div>

最暴力的想法是枚举子数组, 挨个差值判别, 整体上时间复杂度为 $O(n^2)$

而实际上因为与运算的性质, 与运算的结果不会变大, 因此比赛的时候想着使用滑动窗口进行优化枚举过程, 这样做的难点在于, 右边界的滑窗很好运算, 直接运算即可, 但是左边界的滑窗就麻烦的多了

这里使用二进制数组 freq 统计每个 bit 中 1 出现的频率, 显然只有当出现频率恰好等于窗口大小时, 该 bit 在整个子数组中与运算的结果才能是 1
...
class Solution {
    ...
    public int minimumDifference(int[] nums, int k) {
        ...
        int rst = INF;
        int[] freq = new int[32];
        int ands = Integer.MAX_VALUE;
        for (int l = 0, r = 0; r < n; r ++) {
            for (int j = 0; j < 32; j ++) {
                if (((nums[r] >> j) & 1) == 1) freq[j] ++;
            }
            ands &= nums[r];
            rst = Math.min(rst, Math.abs(ands - k));
            if (rst == 0) return rst;
            while (ands < k && l < r) {
                int sum = 0;
                for (int j = 0; j < 32; j ++) {
                    if (((nums[l] >> j) & 1) == 1) freq[j] --;
                    if (freq[j] == r - l) sum |= (1 << j);
                }
                ands = sum;
                rst = Math.min(rst, Math.abs(ands - k));
                if (rst == 0) return rst;
...
而实际上, 这种子数组运算结果存在单调特性的题目, 所有子数组运算可能的取值个数远少于 $O(n^2)$, 这里采用另外的一种枚举方式, 可以在 $O(n\log U)$​ 的时间内完成各种子数组与运算取值的枚举

对于子数组: [1, 2, 3] 而言, 可以通过枚举右边界的方式枚举得到子数组的各个与运算结果:
...
通过这种方式可以在 $O(n^2)$ 的时间内完成各个子数组的与运算的枚举, 同时不需要借助额外的空间 (均为原地修改)

而为了优化这个过程, 可以将枚举时, 如果下一数字枚举时运算结果相比于上一行没有变化直接跳出循环即可, 如下:
...
class Solution {
    ...
    public int minimumDifference(int[] nums, int k) {
        ...
        int n = nums.length;
        for (int i = 0; i < n; i ++) {
            rst = Math.min(rst, Math.abs(nums[i] - k));
            for (int j = i - 1; j >= 0; j --) {
                ...
                // 原地修改
                nums[j] = ne;
                rst = Math.min(rst, Math.abs(nums[j] - k));
            }
        }
...
之所以可以这么剪枝是因为, 当前数字 nums[i] 和 nums[j] 进行与运算后, 结果如果不变, 则 nums[i] 也一定不会修改 j 之前的数字 (与运算越算越小)

这个性质可以进一步推到到或运算 (或运算不会变小), gcd (最大公约数), , lcm (最小公倍数)

>   与运算: 两个数字与运算的结果不会小于两个数字本身, 子数组的与运算具有单调性, 越来越大
>
>   gcd: 当前数字的约数一定不超过当前数字的一半, 参与 gcd 运算的数字越多, gcd 越小, 且每次至少减少为上次的一半, gcd 最多减少 $\log U$ 次
>
>   lcm: 当前数字的倍数至少为当前数字的一倍, 参与 lcm 的数字越多, lcm 越大, 在考虑上界的情况下, lcm 最多增加 $\log U$ 次
>
>   因此这四种运算均可以在 $O(n\log U)$ 的时间内完成子数组运算结果的枚举

要注意的是这种方式只能用于各种运算结果的枚举, 具体每种结果由多少个子数组产生是不能统计得到的

# [第 132 场双周赛](https://leetcode.cn/contest/biweekly-contest-132)

## [100324. 清除数字](https://leetcode.cn/contest/biweekly-contest-132/problems/clear-digits/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/biweekly-contest-132/problems/clear-digits/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/06/09/15:56:53:100324.png" />
	</a>
</div>
...
class Solution {
    public String clearDigits(String s) {
        ...
        int e = 0;
        for (int i = 0; i < n; i ++) {
            if (cs[i] >= 'a' && cs[i] <= 'z') stk[e ++] = cs[i];
            else {
                if (e > 0 && stk[e - 1] >= 'a' && stk[e - 1] <= 'z') e--;
                else stk[e ++] = cs[i];
            }
        }
        return new String(stk, 0, e);
    }
}
```

## [100297. 找到连续赢 K 场比赛的第一位玩家](https://leetcode.cn/contest/biweekly-contest-132/problems/find-the-first-player-to-win-k-games-in-a-row/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/biweekly-contest-132/problems/find-the-first-player-to-win-k-games-in-a-row/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/06/09/15:59:18:100297.png" />
	</a>
</div>

之前的每日一题见过, 对于位置 i 其成为赢家的条件是: 比所有下标小于 i 的数字更大, 且比当前位置向后 k - 1 个数字更大

这里使用了单调栈计算以所有位置 i 为最大值的最有边界, 同时再次遍历找到当前的最大值

注意到如果当前位置 i 比所有前面的位置都大, 但后续数字个数小于 k - 1 个, 那么此时当前位置即为数组的最大值, 直接返回即可
...
class Solution {
    public int findWinningPlayer(int[] skills, int k) {
        ...
        int e = 0;
        for (int i = 0; i < n; i ++) {
            if (skills[i] > skills[max]) max = i;
            while (e > 0 && skills[stk[e - 1]] < skills[i]) {
                int end = stk[-- e];
                right[end] = i;
            }
            stk[e ++] = i;
        }
        while (e > 0) right[stk[-- e]] = n - 1;
		
        // 第一个位置需要比后面 k 个人更大
        if (right[0] >= k + 1) return 0;
		
        // 维护当前枚举到的最大值
        int tmp = 0;
        for (int i = 1; i < n; i ++) {
            if (skills[i] == skills[max]) break;
            tmp = Math.max(tmp, skills[i]);
            if (tmp == skills[i] && right[i] - i >= k) return i;
        }
...
## [100327. 求出最长好子序列 II](https://leetcode.cn/contest/biweekly-contest-132/problems/find-the-maximum-length-of-a-good-subsequence-ii/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/biweekly-contest-132/problems/find-the-maximum-length-of-a-good-subsequence-ii/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/06/09/16:05:10:100327.png" />
	</a>
</div>

周赛的 T3 和 T4 是相同题目不同的输入范围, 对于 T3, 直接定义状态 f\[i][j] 表示以 nums[i] 结尾的, 不满足条件的位置个数不超过 j 个最长子序列; 状态转移以前一个元素的位置分类:

*   如果前一个位置和当前位置相同, f\[i][j] = f\[x][j] + 1
*   如果前一个位置和当前位置不同, f\[i][j] = f\[x][j - 1] + 1

综上需要枚举前一个位置, 因此整体时间复杂度为: $O(n^2\times k)$
    ...
    public int maximumLength(int[] nums, int k) {
        ...
        for (int i = 1; i < n; i ++) {
            for (int j = 0; j <= k; j ++) {
                f[i][j] = 1;
                for (int x = 0; x < i; x ++) {
                    if (nums[i] == nums[x]) f[i][j] = Math.max(f[i][j], f[x][j] + 1);
                    else if (j > 0) f[i][j] = Math.max(f[i][j], f[x][j - 1] + 1);
                }
            }
        }

        int rst = 0;
        for (int i = 0; i < n; i ++) rst = Math.max(rst, f[i][k]);
        return rst;
    }
}
```

T4 输入范围太大了, 直接超时, 从优化 T4 的状态定义入手, 定义状态 f\[x][j] 表示以数字 x 结尾的, 不满足条件位置个数不超过 j 的最长子序列

>   和上面状态定义最大的区别在于这里的第一个维度不再是数字的下标而是实际的数据

对于当前枚举到的数字 x, 考虑状态转移有:

*   f\[x][j] = f\[x][j] + 1, 表示以当前数字 x 结尾的, 不满足条件个数不超过 j 的最长子序列长度可以增加 1 位
*   f\[x][j] = max(f\[y][j - 1]) + 1, 其中 y 从所有和 x 不同的集合中取得

注意到在枚举第二项时, 可以优化枚举, 定义数组 mx, mx[i] 表示以各种数字为结尾的序列中, 不满足条件个数不超过 j 的最长序列的长度: 显然这个值会随着遍历不断增加, 因此直接向后遍历即可

```java
class Solution {
    public int maximumLength(int[] nums, int k) {
        int n = nums.length;
        Map<Integer, List<Integer>> f = new HashMap<>(n);
        for (int i = 0; i < n; i ++) {
            List<Integer> g = new ArrayList<>(k + 1);
            for (int j = 0; j <= k; j ++) g.add(0);
            f.put(nums[i], g);
        }
        ...
        for (int num : nums) {
            List<Integer> g = f.get(num);
            // for (int j = 0; j <= k; j ++) {
            //     int ne = g.get(j) + 1;
            //     if (j > 0) ne = Math.max(ne, mx[j - 1] + 1);
            //     g.set(j, ne);
            //     mx[j] = Math.max(mx[j], ne);
            // }
            for (int j = k; j >= 0; j --) {
                int ne = g.get(j) + 1;
                if (j > 0) ne = Math.max(ne, mx[j - 1] + 1);
...
编码的时候有一些小细节, 因为需要枚举当前数字在各种 j 限制下的条件, 更新 mx 的时候一定要注意从大到小更新, 如果是从小到大更新, 那么可能会使用 f\[x][j - 1] 更新了 f\[x][j], 而实际上 mx[j] 仅应该从 x 之前的数字中获取

>   如果就是想从小到大遍历的话, 就只能在枚举数字的时候备份一下 mx 数组了
>
>   ```java
>   for (int num : nums) {
>       List<Integer> g = f.get(num);
>       int[] mmx = Arrays.copyOf(mx, k + 1);
>       for (int j = 0; j <= k; j ++) {
>           int ne = g.get(j) + 1;
>           if (j > 0) ne = Math.max(ne, mmx[j - 1] + 1);
>           g.set(j, ne);
>           mx[j] = Math.max(mx[j], ne);
>       }
>       f.put(num, g);
...
此外注意到 mx[j] 本身的定义和上述状态转移方程并不完全一致, 上述状态转移方程中, 明确了需要从所有和 x 不相等的 y 中转移而来, 但实际的 mx[j] 可能是以前面的某个 x 结尾的, 这其实不会影响状态的计算, 因为状态计算中 f\[x][j] = max(f\[x][j], f\[y][j - 1]) + 1, 在 y 取 x 的情况下, 等效为: max(f\[x][j], f\[x][j - 1]) + 1, 状态 f\[x][j] 是一个比 f\[x][j - 1] 更宽的限制, 在取最大值的时候不会状态转移结果的正确性

# [第 401 场周赛](https://leetcode.cn/contest/weekly-contest-401)

## [100325. 找出 K 秒后拿着球的孩子](https://leetcode.cn/contest/weekly-contest-401/problems/find-the-child-who-has-the-ball-after-k-seconds/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/weekly-contest-401/problems/find-the-child-who-has-the-ball-after-k-seconds/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/06/09/17:39:52:100325.png" />
	</a>
</div>

对于一个长度为 n 的数组, 需要 n - 1 秒将球运到结尾, 将其定义为一轮, 那么其实就是看看一共可以运多少轮
...
class Solution {
    public int numberOfChild(int n, int k) {
        int x = k / (n - 1);
        int re = k % (n - 1);
        if ((x & 1) == 1) return n - 1 - re;
        else return re;
    }
}
```

## [100305. K 秒后第 N 个元素的值](https://leetcode.cn/contest/weekly-contest-401/problems/find-the-n-th-value-after-k-seconds/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/weekly-contest-401/problems/find-the-n-th-value-after-k-seconds/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/06/09/17:30:50:100305.png" />
	</a>
</div>

输入范围很小模拟可以过, 但这里要说的是 $O(1)$ 解法

本题使用观察法, 矩阵 matrix\[i][j] = matrix\[i - 1][j] + matrix\[i][j - 1], 通过该状态联想到杨辉三角, 只不过当前矩阵为一个斜放的杨辉三角

第 k 行的第一列为杨辉三角的第 k 行的第一列, 而第 k 行的第二列为杨辉三角第 k + 1 行的第二列, 因此第 k 行的第 n 列杨辉三角第 k + n 行的第 n 列

```java
class Solution {
    // 杨辉三角 -> 组合数 -> C_{n}^{k} = C_{n - 1}^{k} + C_{n - 1}^{k - 1}
    private static final int N = 2020;
    private static final int[][] buff;
    private static final int MOD = (int)1e9 + 7;
    static {
        buff = new int[N][N >> 1];
        for (int i = 1; i < N; i ++) buff[i][0] = 1;
        for (int i = 2; i < N; i ++) {
            buff[i][1] = i;
            for (int j = 2; j <= (i >> 1); j ++) {
                buff[i][j] = (buff[i - 1][j - 1] + buff[i - 1][Math.min(i - 1 - j, j)]) % MOD;
            }
        }

    }
    public int valueAfterKSeconds(int n, int k) {
        return buff[k + n - 1][Math.min(n - 1, k)];
    }
}
```
使用组合数计算杨辉三角, 有 $C_{n}^{k} = C_{n - 1}^{k - 1} + C_{n - 1}^{k}$

> 这个公式的推到之前就说过, 通过类似 dp 的思想考虑, 对于最后一个数字, 如果不选则有 $C_{n - 1}^{k}$ 种选法, 如果选则有: $C_{n - 1}^{k - 1}$ 种选法, 两者直接相加即可

## [100319. 执行操作可获得的最大总奖励 I](https://leetcode.cn/contest/weekly-contest-401/problems/maximum-total-reward-using-operations-i/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/weekly-contest-401/problems/maximum-total-reward-using-operations-i/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/06/09/17:24:16:100319.png" />
	</a>
</div>

> 状态定义有问题导致 TLE

定义状态 f\[i][j] 表示前 i 个数字可以是否可以构成奖励数 j, 考虑状态转移时可以根据是否选择当前数字判断:

* 如果不选择当前数字, 则 f\[i][j] = f\[i - 1][j]
* 如果选择当前数字, 则 f\[i][j] = f\[i - 1][j - nums[i]], 其中需要满足条件 0 <= j - nums[i] < nums[i]

当前状态为上述两种状态的并集

```java
class Solution {
    private static final int N = 4040;
    public int maxTotalReward(int[] rewardValues) {
        Arrays.sort(rewardValues);
        int n = rewardValues.length;
        boolean[][] f = new boolean[n][N];
        f[0][0] = true;
        f[0][rewardValues[0]] = true;
        
        for (int i = 1; i < n; i ++) {
            f[i][0] = true;
            for (int j = 0; j < N; j ++) {
                f[i][j] = f[i - 1][j];
                if (j - rewardValues[i] >= 0 && j - rewardValues[i] < rewardValues[i]) f[i][j] |= f[i][j - rewardValues[i]];
            }
        }
...
T4 输入范围更大, 需要 bitset 优化, 不写了

# [第 402 场周赛](https://leetcode.cn/contest/weekly-contest-402/)

## [100301. 构成整天的下标对数目 II](https://leetcode.cn/contest/weekly-contest-402/problems/count-pairs-that-form-a-complete-day-ii/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/weekly-contest-402/problems/count-pairs-that-form-a-complete-day-ii/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/06/16/15:17:25:100301.png" />
	</a>
</div>

> T1 和 T2 题目一样就是输入范围不同, 不写了

要凑成 24 的倍数, 将原数组修改为 24 的模数, 同时维护每个模数的出现次数即可
...
class Solution {
    public long countCompleteDayPairs(int[] hours) {
        ...
        int[] freq = new int[24];
        for (int i = 0; i < n; i ++) {
        	// 统计可以和当前位置凑成 24 的位置个数
            rst += freq[(24 - hours[i]) % 24];
            freq[hours[i]] ++;
        }
        return rst;
    }
...
## [100316. 施咒的最大总伤害](https://leetcode.cn/contest/weekly-contest-402/problems/maximum-total-damage-with-spell-casting/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/weekly-contest-402/problems/maximum-total-damage-with-spell-casting/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/06/16/15:20:49:100316.png" />
	</a>
</div>

> 打家劫舍值域版

整体逻辑和一般的打家劫舍相似, 区别在于需要维护的是前后两个取值相近的咒语的关系, 因此需要对整个咒语数组排序并去重

```java
class Solution {
    public long maximumTotalDamage(int[] power) {
        int[] arr = Arrays.stream(power).sorted().distinct().toArray();
        int n = arr.length;
        Map<Integer, Integer> m = new HashMap<>(n);
        for (int p : power) m.put(p, m.getOrDefault(p, 0) + 1);
        // f 针对 arr 数组而不是 power
        long[] f = new long[n];
        for (int i = 0; i < n; i ++) {
        	// 不选
            if (i > 0) f[i] = f[i - 1];
            // 选
            long tmp = (long)arr[i] * m.get(arr[i]);
            // 找前驱
            int j = i - 1;
            for (; j >= 0 && arr[i] - arr[j] <= 2; j --);
            if (j >= 0) tmp += f[j];
...
## [100317. 数组中的峰值](https://leetcode.cn/contest/weekly-contest-402/problems/peaks-in-array/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/weekly-contest-402/problems/peaks-in-array/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/06/16/15:34:21:100317.png" />
	</a>
</div>
...
要注意的是树状数组下标从 1 开始, 因此下标需要一个 + 1 的偏移
    ...
    public List<Integer> countOfPeaks(int[] nums, int[][] queries) {
        ...
        for (int[] q : queries) {
            if (q[0] == 1) {
                ...
                else rst.add(query(q[2]) - query(q[1] + 1));
            }
            else {
            	// 修改位置 i 的数值只有可能影响位置 i - 1, i, i + 1 处的峰值特性, 因此这里先重置, 再重新计算 => 避免复杂的 if 分支判别
...
# [第 133 场双周赛](https://leetcode.cn/contest/biweekly-contest-133/)
...
<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/biweekly-contest-133/problems/count-the-number-of-inversions/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/06/23/16:46:16:100333.png" />
	</a>
</div>

比赛的时候找了好久, 一般定义状态 f\[i][j] 表示前 i 个数字, 构成共 j 的逆序对的排列数目 => 网上的解析多为考虑数字 i 可以产生的逆序个数

本题有 requirement 的限制, 不过状态定义还是和一般的 f\[i][j] 相同, 考虑状态转移时, 考虑第 i 个数字可以带来的逆序数目, 最小为 0 个, 最多为 i 个, 假设当前数字可以带来 k 个逆序, 则 $f[i][j] = \sum_{k = 0}^{i} f[i - 1][j - k]$

但要注意, 在本题的设定中, requirement 对方案进行了限制, 如果当前枚举的方案 i 的前一个方案 i - 1 在 requirement 中出现了, 那么就不需要枚举了, 直接加上就好
...
class Solution {
    ...
    public int numberOfPermutations(int n, int[][] requirements) {
        ...
        req[0] = 0; 
        this.buff = new int[n][req[n - 1] + 1];
        for (int i = 0; i < n; i ++) Arrays.fill(buff[i], -1);
        // 本题输入保证 requirements 中肯定有 n - 1, 所以 req[n - 1] 一定不是 -1
    ...
    private int dfs(int i, int j) {
        ...
        if (req[i - 1] != -1) {
            // 前序有限制, 直接加
            if (j - req[i - 1] <= i && j >= req[i - 1]) rst = dfs(i - 1, req[i - 1]);
        } else {
            // 前序没有限制, 枚举
            for (int x = 0; x <= Math.min(i, j); x ++) {
                rst = (rst + dfs(i - 1, j - x)) % MOD;
            }
        }
...
# [第 403 场周赛](https://leetcode.cn/contest/weekly-contest-403/)

## [最大化子数组的总成本](https://leetcode.cn/contest/weekly-contest-403/problems/maximize-total-cost-of-alternating-subarrays/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/weekly-contest-403/problems/maximize-total-cost-of-alternating-subarrays/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/06/23/16:55:25:100337.png" />
	</a>
</div>

比赛的时候满脑子想的都是枚举当前位置 i 接在前序子数组结尾/单独分割子数组的情况, 通过枚举区间的方式计算, 整体时间复杂度到达 $O(n^2)$ 级别, 输入 $10^5$ 的情况下肯定过不了

这里的状态定义 f\[i][0] 表示从位置 i 开始考虑后续所有数字, 如果希望接在前序子数组结尾时, 且不需要变号的最大值; 而 f\[i][1] 表示从位置 i 开始考虑后续所有数字, 如果希望接在前序子数组结尾则必须要变号的最大值
...
class Solution {
    ...
    private long dfs(int i, int j) {
        ...
        long rst = 0;
        if (j == 0) {
            // 如果当前位置不变号, 则下一个位置肯定变号
            rst = dfs(i + 1, 1) + nums[i];
        } else {
            // 如果当前位置变号, 则下个位置可变 (当前位置独立分割子数组) 可不变 (当前位置不独立分割子数组)
            rst = Math.max(dfs(i + 1, 1) + nums[i], dfs(i + 1, 0) - nums[i]);
        }
        f[i][j] = rst;
...
# [第 404 场周赛](https://leetcode.cn/contest/weekly-contest-404/)

## [找出有效子序列的最大长度 II](https://leetcode.cn/contest/weekly-contest-404/problems/find-the-maximum-length-of-valid-subsequence-ii/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/weekly-contest-404/problems/find-the-maximum-length-of-valid-subsequence-ii/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/06/30/16:13:11:100358.png" />
	</a>
</div>

注意题目要求的子序列特征, (sub[0] + sub[1]) % k == (sub[1] + sub[2]) % k => 等价于 sub[0] % k == sub[2] % k

所以题目的意思已经很显然了, 就是求一个最多由两种数字组合的, 不断重复的最长子序列 => 注意如果 sub[0] == sub[1] 也可以, 此时只有子序列中只包含种数字

比赛的时候采用的正向求解, 枚举选择的两种数字 i, j, 然后通过优化的二分查找, 前向找到最长的子序列
    ...
    public int maximumLength(int[] nums, int k) {
        ...
        for (int i = 0; i < k; i ++) {
            ...
            for (int j = 0; j < k; j ++) {
                ...
                while (list.get(ne).get(list.get(ne).size() - 1) > pre) {
                    int l = 0, r = list.get(ne).size() - 1;
                    while (l < r) {
                        int mid = l + ((r - l) >> 1);
                        if (list.get(ne).get(mid) <= pre) l = mid + 1;
                        else r = mid;
                    }
                    cnt ++;
                    pre = list.get(ne).get(l);
                    if (ne == i) ne = j;
                    else ne = i;
                }
...
这还是在使用二分优化后的效果, 如果暴力枚举的话, 肯定是过不了的

本题更好的思路是使用 dp 迭代更新结果, 使用当前数字更新序列, 定义状态 f\[i][j] 表示以 (i, j) 作为重复数字的最长子序列, 对于当前数字 x, 显然由 f\[y][x] = f\[x][y] + 1 => 其中 y 需要枚举
...
## [合并两棵树后的最小直径](https://leetcode.cn/contest/weekly-contest-404/problems/find-minimum-diameter-after-merging-two-trees/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/weekly-contest-404/problems/find-minimum-diameter-after-merging-two-trees/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/06/30/15:57:55:100318.png" />
	</a>
</div>

比赛的时候想法是: 对于每棵树, 计算每个节点到当前树中最远点的距离, 枚举得到距离的最小值, 则新树的直径必定为两个最小值的和

但当时忽略了一种情况: 可能存在一颗大树和一颗小树的情况, 此时合并后最小直径为原来树的直径 => WA 了

因此这就涉及到计算树中各个节点到其余节点距离的最小值 => 两次遍历 => 第一次计算以 0 为根节点时, 各个节点到叶子节点的最大距离; 第二次采用换根 dp, 计算每个节点到树中最远节点的距离

所以比赛的时候最后就是两次 DFS + 两次换根, 最后再和两个树本身的直径取一个交集
...
本题还可以进一步分析, 即两个树相连的点必定在各自的直径上, 且必定为各自的中点:

* 如果不在树的直径上, 那么显然, 由于树的直径是整棵树上最长的距离, 如果当前节点 x 不在直径上, 必然存在位于直径上的节点 y, 满足 x 到直径上的距离即为 x 到 y 的距离, 此时 x 在到树中最远的距离必然大于直径的一半
* 根据上面一点通过优化节点的位置, 只有在节点为数直径的中点时, 当前节点到树中最远的点距离为树直径的一半 => 此时最小

因此为了满足新树直径最小, 连接的两个节点必然为直径上的中点, 此时距离为原来两个子树直径的一半的和

因此本题可以被简化为分别求解两棵树的直径, 取半 (记得向上取整), 求和, 即可得到新树的最小直径

所以现在的关键变为了求树的直径 => 直接套模板, 一次 dfs 即可
...
class Solution {
    ...
    public int minimumDiameterAfterMerge(int[][] edges1, int[][] edges2) {
        int l = diameter(edges1), r = diameter(edges2);
        return Math.max((l + 1) / 2 + (r + 1) / 2 + 1, Math.max(l, r));
    }
...
# [第 134 场双周赛](https://leetcode.cn/contest/biweekly-contest-134/)

## [子数组按位与值为 K 的数目 (Number of Subarrays With AND Value of K)](https://leetcode.cn/classic/problems/number-of-subarrays-with-and-value-of-k/description/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/classic/problems/number-of-subarrays-with-and-value-of-k/description/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/07/07/15:37:14:100338.png" />
	</a>
</div>

同样是与运算, 同样是子数组, 因为需要枚举各个子数组与取值, 这里需要使用之前[周赛](#[100315. 找到按位与最接近 K 的子数组](https://leetcode.cn/contest/weekly-contest-400/problems/find-subarray-with-bitwise-and-closest-to-k/))通过位运算加速枚举子数组与运算的结果, 外层循环枚举以 i 结尾子数组, 内层循环计算由 j 开头到 i 结尾的子数组的与运算

先比于之前的题目, 这里需要计算子数组的个数, 直接二分找边界即可
...
# [第 135 场双周赛](https://leetcode.cn/contest/biweekly-contest-135)

## [使差值相等的最少数组改动次数](https://leetcode.cn/contest/biweekly-contest-135/problems/minimum-array-changes-to-make-differences-equal/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/biweekly-contest-135/problems/minimum-array-changes-to-make-differences-equal/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/07/21/16:15:00:100365.png" />
	</a>
</div>

本题从枚举数对对差值的 X 的贡献出发, 考虑对于数对 (i, j), 如果当前差值为 nums[j] - nums[i] (nums[j] > nums[i]), 显然在最终差值小于 nums[j] - nums[i] 时只需要变换一个数字即可, 随着差值不断变大, 最终需要变换两个数字, 改数对在只修改一个数字能够达到的差值为 max(k - nums[i], nums[j])

对于数组中的每个数对 (i, j) 都是这样, 在最终的 X 小于某个数字时, 只需要修改一个位置, 而在大于某个数字时, 需要修改两个位置

使用差分数组计算每个数对对最终的差值的贡献, 因为需要对区间更新, 使用差分数组优化
...
class Solution {
    ...
    public int minChanges(int[] nums, int k) {
        ...
        for (int i = 0, j = n - 1; i < j; i ++, j --) {
            ...
            int x = max - min;
            // 变为 0 ~ x - 1 时只需要修改一个位置 (变为 x 不需要修改位置)
            d[0] += 1;
            d[x] -= 1;
            // 变为 x + 1 ~ m 时也只需要修改一个位置
            d[x + 1] += 1;
            int m = Math.max(max, k - min);
            // 变为 m ~ k 时需要修改两个位置
            // d[m + 1] -= 1;
            // d[m + 1] += 2;
            d[m + 1] += 1;
...
## [使两个整数相等的位更改次数](https://leetcode.cn/contest/weekly-contest-407/problems/number-of-bit-changes-to-make-two-integers-equal/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/weekly-contest-407/problems/number-of-bit-changes-to-make-two-integers-equal/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/07/21/16:29:00:100372.png" />
	</a>
</div>

除了直接按位枚举, 还可以使用位运算, 从二进制集合的角度, k 必须为 n 的子集, 才能将 n 变为 k, 此时返回值为补集的大小
...
## [字符串元音游戏](https://leetcode.cn/contest/weekly-contest-407/problems/vowels-game-in-a-string/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/weekly-contest-407/problems/vowels-game-in-a-string/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/07/21/16:31:47:100335.png" />
	</a>
</div>

移除的子串不需要只包含元音字母, 只要移除的子串中至少含有一个元音字母就好了

对于原字符串而言, 如果中包含奇数个元音字符, 那么先手直接全部移除即可, 而如果包含偶数个元音字符, 那么在先手移除了包含奇数个元音字符的子串后, 剩下整个字符串中肯定剩下奇数个元音字符 => 后手无法一次性移除

因此只要原字符串中包含了元音字符, 就一定可以获胜
...
## [将 1 移动到末尾的最大操作次数](https://leetcode.cn/contest/weekly-contest-407/problems/maximum-number-of-operations-to-move-ones-to-the-end/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/weekly-contest-407/problems/maximum-number-of-operations-to-move-ones-to-the-end/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/07/21/16:37:27:100360.png" />
	</a>
</div>

本题的移动规则是, 1 必须一次性经过所有连续的 0; 因为 0 的总数是确定的, 为了让操作数最大, 应该尽可能避免 0 连续

如果从后向前更新 1, 因为此时会将排列靠后的 0 换到前面去, 可能导致连续 0 区间的合并, 从而减少了前面其余 1 的操作次数, 此时计算得到的是最小操作次数

如果从前向后更新 1, 尽管此时还是有 0 被换到前面去, 但是此时前面连续的 0 不会继续参加后面 1 的操作中了, 此时计算得到的是最大操作次数
...
## [使数组等于目标数组所需的最少操作次数](https://leetcode.cn/contest/weekly-contest-407/problems/minimum-operations-to-make-array-equal-to-target/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/weekly-contest-407/problems/minimum-operations-to-make-array-equal-to-target/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/07/21/16:46:38:100329.png" />
	</a>
</div>

将一个数组转变为另一个数组, 不难想到对两个数组取差值, 此时得到数组 arr, 从而将本题转化为, 如何通过最少次的区间更新将 arr 数组变为全 0 表示

涉及到区间更新, 会联想到使用差分数组, 将区间更新转化为两个边界的更新, 进一步将 arr 转化为差分数组 diff

对于区间 [i, j] 如果希望将区间整体增加 1, 等价为进行 diff[i] + 1, diff[j + 1] - 1 两步操作, 由此可见, 每次区间修改可以同时满足一个位置增加, 一个位置减少的需求

为了将差分数组置为全 0 数组 (差分数组全为 0, 原数组也必然全为 0) => 维护两个指针分别表示差分数组的下一个负数位置, 与下一个正数位置, 合并二者的修改
...
# [第 408 场周赛](https://leetcode.cn/contest/weekly-contest-408/)

## [统计 1 显著的字符串的数量](https://leetcode.cn/contest/weekly-contest-408/problems/count-the-number-of-substrings-with-dominant-ones/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/weekly-contest-408/problems/count-the-number-of-substrings-with-dominant-ones/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/07/28/20:08:05:100348.png" />
	</a>
</div>

本题的输入范围达到了 $4\times 10^4$, 显然枚举子数组左右端点的方式肯定会 TLE 的, 本题的突破口是题干, 一个显著字符串中 1 的数量大于 0 的数量的平方, 假如 0 的个数为 x, 1 的个数必须至少为 $x^2$, 则子串长度为 $x + x^2$, 因此在本题的显著子字符串中, 0 的个数一定不会超过 200 个

如果在枚举子字符串左边界的同时, 枚举子字符串中 0 的个数, 则可以将时间复杂度降低为 $O(n\sqrt n)$

如果当前枚举到的字符串中 0 的个数为 y, 对应的下标为 i, 而子字符串中 0 的个数为 y + 1 时对应下标为 j, 则当前左边界下, 所有包含了 y 个 0 的字符串一共有 j - i 个

然而本题还有一个限制, 即子字符串中 1 的个数必须大于 0 的个数的平方, 因此对于当前枚举的 0 的个数 y, 一定可以确定最短的子字符串长度 $len = y + y^2$, 如果 $len \geq j$  则在当前子字符串左边界的情况下, 不存在任何的显著子字符串
...
class Solution {
    public int numberOfSubstrings(String s) {
        ...
        int x = 0;
        for (int i = 0; i < n; i ++) {
            // 在 i 作为左边界的情况下, 枚举 0 的位置必须至少在 i 之后 
            while (x < l.size() && l.get(x) < i) x ++;
			
            // 在当前位置 i 为 1 的情况下, 对字符串中只包含了 0 个 0 的情况进行枚举
            if (cs[i] == '1') {
                if (x == l.size())
                    rst += n - i;
                else
            ...
            // 枚举子字符串中 0 的个数 (从 1 开始, 最多枚举到 200 个)
            for (int j = 1, y = x; j * j < n && y < l.size(); j ++, y ++) {
                // 计算从左边界到恰好包含了 j 个 0 的字符串的长度
                int len = l.get(y) - i + 1;
                // 计算从恰好包含 j 个 0 到恰好包含 j + 1 个 0 的字符串长度
                int re;
                if (y < l.size() - 1)
                    re = l.get(y + 1) - l.get(y);
                else
                    re = n - l.get(y);
				
                int min = j * j + j;
                // 如果最长的子字符串也不够长, 说明在当前左边界包含了 j 个 0 的子字符串下, 不存在显著子字符串
                if (min >= len + re)
                    continue;
                // 只有长度至少为 j * j + j 才能纳入结果集
                rst += len + re - Math.max(min, len);
            }
        }
        return rst;
    }
...
## [新增道路查询后的最短距离 II](https://leetcode.cn/contest/weekly-contest-409/problems/shortest-distance-after-road-addition-queries-ii/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/weekly-contest-409/problems/shortest-distance-after-road-addition-queries-ii/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/08/04/16:05:35:100376.png" />
	</a>
</div>

每次添加的区间不存在交叠, 这意味着每次进行从 [l, r] 的边的添加后, 可以将 [l, r] 的距离缩短为 1

每次询问的是到终点的距离, 可以等价于从起点到终点最少经过的区间个数, 初始情况下, 一共有 n - 1 个区间, 随着合并不断进行, 区间数减少

使用有序集合维护区间终点, 并在新边添加后, 删除部分区间

```java
class Solution {
    public int[] shortestDistanceAfterQueries(int n, int[][] queries) {
        TreeSet<Integer> set = new TreeSet<>();
        // 默认存在 n - 1 个区间, 区间边界分别为 [1, 2, 3, ..., n - 1]
        for (int i = 1; i < n; i ++) 
            set.add(i);
        ...
        int[] rst = new int[m];
        for (int i = 0; i < m; i ++) {
        	// 每个 [l, r] 会删除的区间边界为: [l + 1, l + 2, ..., r - 1]
            int x = queries[i][0] + 1;
            while (x < queries[i][1]) {
            	// 每次迭代的下一个区间边界通过查询获得 (如果直接遍历的话删除的时间复杂度达到了 n, 整体时间达到 mn)
                int ne = set.ceiling(x);
                if (ne < queries[i][1])
                    set.remove(ne);
                x = ne;
            }
...
## [单调数组对的数目 II](https://leetcode.cn/contest/weekly-contest-410/problems/find-the-count-of-monotonic-pairs-ii/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/contest/weekly-contest-410/problems/find-the-count-of-monotonic-pairs-ii/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/08/11/14:49:11:100396.png" />
	</a>
</div>
这次周赛的 t3 和 t4 的题目描述完全一致, 唯一的区别在于输入范围存在差异, 其中 t3 的 nums[i] 取值不超过 50, 而 t4 的输入达到了 1000; 从某些程度上, 其实也是在按时本题的状态定义可能和 nums[i] 的数值有关

根据题目描述, arr1[i] + arr2[i] = nums[i], 也就是说枚举 arr1[i] 时, 不需要再枚举 arr2[i] 了, 可以直接通过做差得到

定义状态 f\[i][j] 表示前 i 个位置, 第 i 个位置在 arr1[i] 取值为 j 时的数对数目, 最终返回值为 f\[n - 1][0] + f\[n - 1][1] + ... + f\[n - 1][nums[n - 1]]

状态转移从枚举前一个位置在 arr1 中的数字 k 考虑, 显然 0 <= k <= nums[i - 1]

当前位置 i 在 arr1 中如果取到了 j, 那么显然前一个位置 i - 1 处在满足 arr1 非递减的情况下, 必须满足 k <= j; 除了 arr1 需要满足条件, arr2 也需要满足非递增的条件, 即满足 nums[i - 1] - k >= nums[i] - j, 从而有 k <= nums[i - 1] - nums[i] + j

所以在枚举前一个位置的数字 k 时, 其下限一定为 0, 而其上限会受三个方面的影响, 总和取 min => min(nums[i - 1], j, nums[i - 1] - nums[i] + j)

对于 t3 而言, 直接枚举即可, 整体时间复杂度达到了 $O(n\times m^2)$, m 取到 50 的情况下, 能过, 但 t4 的输入范围更大, 这里需要考虑优化枚举 k 的维度

因为对于 k 的枚举都是从 0 开始的, 完全可以通过前缀和进行优化, 将枚举 k 的过程变为 $O(1)$ 的前缀和计算
    ...
    public int countOfPairs(int[] nums) {
        ...
        for (int i = 1; i < n; i ++) {
            for (int j = 0; j <= nums[i]; j ++) {
                int lim = Math.min(nums[i - 1], j);
                lim = Math.min(lim, nums[i - 1] - nums[i] + j);
            ...
            // 前缀和的计算需要转移到底, 从 nums[i] + 1 开始计算到 M
            for (int j = Math.max(1, nums[i] + 1); j < M; j ++) 
                pre[i][j] = pre[i][j - 1];
            
        }
        return pre[n - 1][nums[n - 1]];
    }
}
```
