...
这个注解用来从配置文件(可能是 .yaml 也可能是 .properties)中读取配置到 java bean
...
默认的 spring boot 使用过 setter 注入配置的，所以 setter 方法一定要存在，当然，借助 lombok 实现的 java bean 也是可以的

当然可以额外配置通过构造方法注入属性，这些看官网就行了

## 注入 bean

上面仅仅是创建了一个 java 对象，并将 java 对象的属性和配置文件中的属性进行了绑定，如果需要让 spring 接管这个对象，显然是需要将其注册为一个 bean 的

最简单的方法，添加一个 @Component 注解即可

>   在 spring boot 中不需要主动配置 component scan 就可以扫描到 bean，这是因为程序的入口函数中添加了 @SpringBootApplication 注解，这个注解是一个复合注解，里面配置了 component scan 为当前目录

不过显然，如果配置类比较多的话，这样注册 bean 并不方便管理，所以实际中是在 xxxxConfiguration 中将上面的对象注册为 bean

可以通过 @bean 并且 new 对象的方式注册 bean，也可以直接在配置类上添加 @EnableConfigurationProperties() 的方式注册对应的 bean

注入 bean 之后，如果需要在某个地方使用配置，直接通过 @AutoWired 获取 bean 就好了

## 增加编码的提示

配置好 bean 并使用 IDEA 写配置文件的时候，发现没有自动补全，这么多属性，我怎么可能都记住
...
其实就是引入了一个注解处理器，处理的就是注解 @ConfigurationProperties，官网在 [Configuration Metadata](https://docs.spring.io/spring-boot/docs/2.7.3/reference/html/configuration-metadata.html#appendix.configuration-metadata.annotation-processor)

官网里说了，如果项目中还是用了 lombok(一个插件，也是依赖于注解处理器的)，那么此时，要保证 lombok 的注解处理器运行在 spring-boot-configuration-processor 之前，为了实现这点，官方要求：
