...
* file table(文件表)：所有打开文件的集合组成的一张表，被所有进程共享。每一个 entry 对应了一个文件，包含了文件位置，reference count(引用计数)，和一个指向 v-node 表中某个 entry 的指针

    >   这里的文件位置，不是那个存储位置，而是一个字节偏移量
    >
    >   reference count 表示有多少个 descriptor table 中的 entry 指向了当前 file table 中的 entry
    >
...
对于第一个例子，一个进程中 open 了两个 file，对应了两个 fd，即进程 context 中 descriptor table 的两个 entry，这两个 entry 又指向了实际 file table 中的两个 entry，在 file table 中可以看到文件位置信息和文件的 reference count 信息，同时这两个 entry 还指向了 v-node table 中的两个 entry，v-node table 相比于 file table 更加详细的描述了文件信息

第二个例子，相对于第一个例子最大的区别在于，两次 open 打开的是相同的文件，在 file table 中可以看到两个 entry 指向了 v-node table 中相同的 entry，尽管 descriptor table 的 entry 不同(因为两次 open)，尽管 file table 的 entry 不同(因为文件位置不同)，但是最终指向的是 v-node table 的相同的文件

>   比如先写再读一个文件，或者从两个位置读同一个文件；linux 不建议先读后写，因为这样可能导致写操作覆盖掉之前读的内容，但 linux 支持这样的操作

还记得之前说过父子进程共享了 file descriptors，其实说的是通过 fork 创建的子进程，在其 context 中也保存了一个和父进程取值相同但相互独立的 descriptor table，如下图所示

![](https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/child_inherite_parent_descriptor_table_emp.png)

fork 一个子进程后 file table 的 reference count 会自增，但因为两个子进程使用的是 file table 中相同的 entry，因此此时是存在隐患的，比如父进程如果先对一个文件进行 read 操作，导致文件位置变化，随后子进程再次 read，那么起始位置就会从刚刚父进程结束的位置开始

考虑一个文件 foo.txt 中保存了 "abcdefg"，父进程先 read 了文件中的若干字节 3，那么子进程再次 read，将从 d 开始

而程序运行的过程中父子进程的运行是没有先后顺序的，有可能本次是父进程先 read，有可能是子进程先后 read，因此此时的 read 操作的结果是未知的
...
上面将 stdout 重定向到文件 foo.txt，而通过调用 syscall dup2 可以显式的实现 IO 重定向
...
dup2 会在 descriptor table 中 newfd 对应的 entry 通过 oldfd 的覆盖掉，即一次 dup2 后 descriptor table 中 newfd 和 oldfd 中保存的都是原先 oldfd 的 entry

如果 newfd 对应的文件尚未关闭，那么 dup2 会在覆盖前，将 newfd 对应的文件 close，下图给出了 dup2(4, 1) 的操作，即将 stdout 重定向到 fileB 中
...
syscall mmap 会然内核创建一段区域，内核会尽量从 start 开始进行分配，并将 fd 对应的文件的一段连续的 chunck 映射到新区域，fd 中的 chunck 的大小为 length， 且距离文件开头的具有 offset 的偏移量
...
next fit 因为是循环遍历 list，因此对于较大的 block 也可以保证在相对较小的时间内进行匹配，但 "一些研究" 表明，next fit 对于内存的利用率比 first fit 低的多
...
优化的思路是**尽可能减少 footer 的使用**，考虑只对 free 状态的 block 添加 footer，同时将前一个 block 的状态放在当前 block 的某个闲置位中
...
当释放掉一个 block 时，根据当前 block 的header 确定前一个 block 的状态，如果前一个 block 为 allocate 状态，当前 block 一定不可以和前一个 block 合并；而当前一个 block 为 free，可以通过前一个 block 的 footer 确定前一个 block 的大小，从而进行合并
...
伙伴系统是 segregated fit 的特殊情况，每个等价类的大小为 2 的幂次

最开始堆的大小为 $2^m$ Byte，所有对大小为 n 的 block 的请求会向上对 2 的幂次取整，考虑分配一个大小为 $2^k$ ($0\leq k\leq m$)的 block

首先 allocator 会将大小为 $2^m$ 的 block 分割为两个大小为 $2^{m - 1}$ 的 block，其中一个保存到对应 list 中，另外一个继续分为两个大小为 $2^{m - 2}$ 的 block，迭代这个过程最终会得到两个大小为 $2^k$ 的 block，allocator 将其中一个返回给应用程序，将另外一个保存下来，这样 allocator 会保存了大小为 $2^k, 2^{k + 1}, ..., 2^{m - 1}$  的 block，每类一个

当释放一个 block 时，会执行合并，buddy system 最特别的地方是，其可以根据 block 大小和地址直接进行合并，假如现在有一个大小为 32 字节的 block，其地址为 xx00000，那么其 buddy block 一定为 xx10000，在 buddy system 中 block 本身和 buddy block 只有一位是不同的

然而 buddy system 因为不会分割 block，因此将存在大量的内部碎片，所以内存利用率其实不是很高，但如果存在一种用户程序，其会经常请求大小为 2 的幂次内存的话，那么内部碎片的问题也不存在了
