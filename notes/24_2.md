...
## [2673. 使二叉树所有路径值相等的最小代价 (Make Costs of Paths Equal in a Binary Tree)](https://leetcode.cn/classic/problems/make-costs-of-paths-equal-in-a-binary-tree/description/)

<div style="text-align:center;">
	<a href="https://leetcode.cn/classic/problems/make-costs-of-paths-equal-in-a-binary-tree/description/" >
		<img src = "https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/02/28/10:41:58:2673.png" />
	</a>
</div>

从贪心的思路上, 为了让操作的次数最少, 那么最终所有的路径之和都等于原树中最大路径和

对于一个大小为 n 的满二叉树, 其叶子节点的数量为: $m = \frac{n + 1}{2}$, 从最简单的思路考虑, 那么所有路径的修改都可以通过修改叶子节点的值得到, 通过维护一个大小为 m 的数组可以记录使得所有路径和相等时, 各个叶子节点需要修改的大小

对于一个二叉树而言, 修改父节点显然是比修改两个子节点的开销更小的, 如果增加父节点的大小为 x, 就等效也增加两个子节点的大小为 x -> 开销降低了 2x

因此这里为了求解最小开销, 可以考虑增加父节点的值, 已知了各个叶子节点需要增加的大小, 向上抽取的时候父节点只需要增加两个子节点中更小的那个; 注意到这个过程可以不断迭代下去直到根节点 (根节点一定不需要修改)
...
class Solution {
    public int minIncrements(int n, int[] cost) {
        ...
        // tmp 维护了每条路径的开销
        int[] tmp = new int[n];
        System.arraycopy(cost, 0, tmp, 0, n);
        for (int i = 0; i < n; i++) {
            int l = (i << 1) + 1;
            int r = l + 1;
            if (l < n) {
        ...
        System.arraycopy(tmp, n - m, buf, 0, m);
        
        int max = 0;
        for (int num : buf) max = Math.max(max, num);
        int rst = 0;
        for (int i = 0; i < m; i++) {
            // buf 维护了每条路径上需要增加的值
            buf[i] = max - buf[i];
            // rst 记录的是只修改叶子节点的情况下的开销
            rst += buf[i];
        }
        // 从 m 个叶子节点开始, 不断向上迭代
        // 每轮修改在 buf 数组上原地迭代
        for (int e = m; e > 1; e >>= 1) {
            for (int i = 0; i < e >> 1; i++) {
                int l = (i << 1);
                int r = l + 1;
                buf[i] = Math.min(buf[l], buf[r]);
                // 每修改一个父节点, 就等价少修改一个子节点
                rst -= buf[i];
            }
        }
        return rst;
...
## [2581. 统计可能的树根数目 (Count Number of Possible Root Nodes)](https://leetcode.cn/classic/problems/count-number-of-possible-root-nodes/description/)
...
最简单的方法是, 以每个节点为根, 然后检查 guess 数组中满足条件的元素的个数, 这需要以分别每个节点为根遍历整棵树, 整体时间复杂度达到了 $O(n^2)$, 本题输入的输入范围达到了 $10^5$ 显然是过不了的

因此优化的思路是, 首先遍历一棵树, 然后先办法利用之前的结果加速其他树的遍历, 也就是本题中提到的换根 dp

![](https://cdn.jsdelivr.net/gh/buzzxI/img@latest/img/24/02/29/20:30:30:chroot_dp.jpg)

在本题中强调的是父节点和子节点的关系, 现在假设原来的根节点为 root, 在切换为 u 之后, 整棵树中父子关系发生转变的只有节点 r - u, 原来 r 为父节点, 而现在 u 为父节点; r 其余的子节点现在还是其子节点, u 原来的子节点也还是 u 的子节点

因此如果知道了 r 作为根节点时, 满足的父子关系式的个数, 那么在切换到 u 作为根节点时, 根本不需要再遍历整棵树, 可以在 $O(1)$ 的时间内计算增量

因此本题的求解分为两部分, 首先以任意节点为根节点, 求解满足的关系式的个数; 然后从该根节点开始执行换根操作, 每个新换的根都可以在 $O(1)$ 的时间内计算其满足条件的关系式的个数
