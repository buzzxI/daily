...
一般考虑计算数组中前n项的和，或者是求区间和的时候，使用的是前缀和数组，很简单，也很好用
...
假设原来的数组长度为8，下标为1-8，用A表示，对应树状数组下标：1-8，用C表示
...
乍一看好像没什么规律，其实主要是和最低为的1有关

在`7`的二级制中最低为的1就是1（xxx1），所以记忆为`C[7]`可以表示原数组中的1位数

在`6`的二级制中最低为的1是2（xx10），所以记忆为`C[6]`可以表示原数组中的2位数

在`4`的二级制中最低为的1是4（x100），所以记忆为`C[4]`可以表示原数组中的4位数

这里有一个函数，它的作用就是获取到任意一个数字中，最低位的1所代表的数字：
...
我们带入`7`将返回`1`，带入`6`将返回`2`，带入`4`将返回`4`

对于树状数组有这样的性质：

* 每个 C[x] 节点覆盖的节点的个数为 lowbit(x)
* 任意一个节点 C[x] 的父节点为：C[x + lowbit(x)]

第一条上面已经说过了，而至于第二条就是我们画出上面原数组和树状数组对应关系图的根本

给定一个数组，我们可以通过上面的两个性质构造出树状数组

对于数组中的第一个元素`A[1]`我们需要改变的有：

`C[1]`，`C[1 + lowBit(1)], C[1 + lowBit(1) + lowBit(1 + lowBit(1))]...`

那么对于构造树状数组有：

```java
//注意这里的index不是常规意义的数组下标索引，index从1开始
...
所以给定一个原数组，构造树状数组的时间复杂度为$O(n\log n)$，相比于原始的前缀和数组，其实时间复杂度更高，但树状数组的优势在于修改区间
...
可以看到，单点修改的时间复杂度为$O(\log n)$，相比于普通的前缀和数组已经改进了很多了

绕了一大圈，说明了树状数组中的节点和原数组中节点的对应关系，如果要计算前n项的和，假设为7，有`sum = C[7] + C[6] + C[4]`，从图中，很容易得到这个结论，但我们不可能所有的情况都靠画图

其实计算数组中前`n`项的和也是和`lowBit`函数有关的，有：
...
对于单点修改的情况，求出数组中某一点的取值其实是一个伪命题，在单点修改中，我们完全可以同时维护前缀和数组（或树状数组）和原数组，这样，单点求和的时间复杂度一直都是$O(1)$

> 事实上，使用树状数组必须保留一份原数组，在我们上面进行的update操作中，我们需要计算val - num[index]的值得到一个gap，从而更新树状数组

至此，我们说明了树状数组相比于前缀和数组的优势，即：

* 在构造数组时，前缀和数组因为迭代的公式为`preSum[i] = preSum[i - 1] + num[i - 1]`，所以时间复杂度为$O(n)$；而树状数组构造时，对于每一项的加入，改变的节点为`index, index + lowBit(index), ...`最坏情况下有$\log n$项，所以时间复杂度为$O(n\log n)$
...
所以综上，如果求区间和，不修改原数组的情况（或者几乎不修改原数组），使用前缀和数组；如果经常对原数组进行修改，此时考虑使用树状数组可以提升性能
...
这是一个在 $O(n)$ 的时间复杂度下，求解字符串**最长回文子串**的算法(这里没有说最长回文子序列，子序列需要使用 dp 才行)

本质上其实就是中心扩展算法，只不过在计算每个位置的最长回文子串的时候，利用了前一个位置的信息，加速了查找

所谓中心扩展算法，其实很简单，就是一个 while 循环
...
这个循环的意义是，给定了初始的边界，然后求解最长的回文的长度

> 注意到出循环时 left 和 right 一定都是非法值，合理的回文子串的范围为 s[left + 1] 到 s[right - 1]，因为这里因为 substring 方法的特性，没有进行 right - 1的操作
>
> 单纯的中心扩展算法调用这个方法时，left 和 right 参数的关系要么为 right = left(此时回文中心是原字符串中的某个字符)，要么 right = left + 1(此时回文中心是原字符串中某两个字符中间的位置)

为了描述 Manacher 算法，定义臂长 length ：如果一个回文子串的长度为 2 x length + 1，那么称臂长为 length

注意到这里的定义针对的是奇数长度的回文子串，即以某个字符的为回文中心的情况，其实使用 Manacher 算法需要对原来的字符串进行填充，即如果原字符串为 abcde 的形式，需要填充为 xaxbxcxdxex 其中 x 理论上可以是任意字符，甚至可以是原字符串中已经出现过的字符，不过需要注意的是，填充的 x 在一个字符串中必须是一致的，不能前面填充 a 后面填充 b

填充好字符后，如果我们希望知道原字符串中

* 以某个字符为回文中心构成的最长回文子串，直接在对应位置求解即可
* 以某两个字符的中点为回文中心构成的最长回文子串，直接以对应的两个字符中间的填充字符 x 为回文中心求解即可

求出的结果会包含了填充字符 x ，可以很容易去除填充字符(遍历就行)

现在有了填充字符后，我们的回文中心必定只可能是填充后的字符串中的某个字符，要注意的是 Manacher 算法求解回文子串其实是在求解臂长，如果某个位置的臂长更长，那么直接更新 left 和 right 指针进行记录，这里求解某个位置臂长需要利用之前求解的结果加快当前位置的查找
...
注意上图中位置 2j - i 的臂长仅为 1，在实际中，这个 n 可能很大，超出了 j 的最长回文子串的范围，所以当通过对称的方式求解时，臂长是受限的，如下图

![](https://cdn.jsdelivr.net/gh/SunYuanI/img/img/Manacher_2.png)

综上在求解位置 i 的臂长时，我们知道了当前位置的臂长 curLen 至少为 Math.min(n, j + length - i)，那么在调用 expand 方法时，就可以假设 left 为 i - curLen 而 right 为 i + curLen，通过这种方式加快搜索

实际编码的时候，尽管填充的字符 x 理论上可以随便取，不过前人一般都使用 # 进行填充，故这里也遵从这个习惯

这里给出一个最长回文子串的裸题 [5. 最长回文子串](https://leetcode.cn/problems/longest-palindromic-substring/)
...
class Solution {
    /**
     * 注意这个方法求解的结果是臂长
     * 这里的 left 和 right 一定是关于 chars 数组中的某个位置对称的
     * 且最后出循环时 left 和 right 一定都是非法值
     * 那么最后有效的回文子串的长度 len = right - left + 1 - 2
     * 第一个 +1 后得到的是包含了非法值的回文子串的长度
     * 第二个 -2 后得到的是合法的回文子串的长度
     * 因为此时回文子串长度一定为奇数，所以臂长为 (len - 1) / 2，这里使用了位运算
     */
    ...
    public String longestPalindrome(String s) {
        // 使用 chars 数组保存填充后的字符串，长度为 2 * s.length() + 1
		...
		// 用来保存某个位置的臂长
       	int[] dp = new int[len];
		// 使用 start 和 end 刻画最长回文子串的位置
        int start = 0;
        int end = -1;
		// 当前臂长最长的位置 j 和其臂长可到达的最远位置 right
        int right = -1;
        int j = -1;
        // 求解每个位置的臂长
        for (int i = 0; i < len; i++) {
            int curLen = 0;
            // 当 j 有效时，可以通过对称的性质加快查找，而当 j 无效时，就需要从当前位置硬扩展了
            if (right > i) {
                int length = Math.min(list.get(2 * j - i), right - i);
                curLen = expand(chars, i - length, i + length);
            } else {
                curLen = expand(chars, i, i);
            }
            // 不要忘了记录当前位置的臂长
            dp[i] = curLen;
			// 如果当前位置可以到达的最远位置已经超过了 j 那么需要更新 j
            if (i + curLen > right) {
                j = i;
                right = i + curLen;
            }
			// 如果当前位置的最长回文子串的长度已经超过了记录的最大值需要更新两个指针
            if (2 * curLen + 1 > end - start + 1) {
                start = i - curLen;
                end = i + curLen;
            }
        }
        // 根据两个指针的指示，从填充后的数组中恢复原来的回文子串
        StringBuilder builder = new StringBuilder();
        for (int i = start; i <= end; i++) {
            if (chars[i] != '#') builder.append(chars[i]);
        }
...
注意到这里的关键点其实在于影子状态 x，默认初始化为 0；而我们当前状态最开始也为 0，在循环迭代的时候从 1 状态开始

所以实现 KMP 其实很简单：

* 定义 dp 数组，称 `dp[i][j]` 为当处于状态 i 输入为 j 时，下一个转移到的状态
* 定义好影子状态 x，默认为 0；当前状态从 1 开始遍历

下面给出一个 KMP 的裸题 [28. 找出字符串中第一个匹配项的下标](https://leetcode.cn/problems/find-the-index-of-the-first-occurrence-in-a-string/)
...
网上一些其他的 KMP 说的都是使用 next 数组，而不是一个二维的 dp，也许就是因为空间太大了，所以才过不了吧

关键在于 next 数组，next[i] 表示以 i 结尾的后缀中和 pat 串前缀相等的最大值

如果 next[i] = j, 说明 pat 串从 0 -> j 和从 i - j -> i 相等

编码细节上，遵从 y 总的习惯，即字符串从 1 开始，且每次匹配的是 i 和 j + 1

>   即之前匹配的时候，i - 1 和 j 对应，且 str[i - 1] = pat\[j] (str 从 i - j + 1-> i - 1 和 pat 从 1 -> j - 1 是匹配的)
>
>   如果 str[i] != pat[j + 1]，那么对于 j 进行回退，即让 j = next[j]；回退后继续比较 pat[j + 1] 和 str[i]，如果二者不相等，那么需要继续迭代，回退，直到回退到 j 取 0；此时再比较 j + 1 和 str[i] 的时候，相当于 str 开始从头和 pat 比较
...
这是一类枚举子序列的题目，很经典，如果不知道的话，只能暴力回溯，显然指数的时间复杂度是不能接受的，需要考虑其他方法

## 元素非负

首先考虑元素都是非负的简单情况：

如果数组为：[$a_1,a_2,a_3...$]\(这里已经按照大小顺序进行排序了)，那么最小的子序列(如果不包括子序列为空的情况的话)就是数组中的第一个元素，而最大的就是整个数组本身，但是应该如何找到第 k 小的子序列呢：

这里定义一个数对 (s, i) 表示子序列总和为 s，且子序列中最后一个元素为 nums[i] 的子序列的和；维护一个存储了(s, i) 对的小顶堆，小顶堆按照数对的第一个维度进行排序

最开始让小顶堆中存储 (nums[0], 0)，表示最小的子序列

当从小顶堆中取出数对 (s, i) 时，存放进入的堆的数对应该是(s + nums[i + 1], i + 1) 和 (s - nums[i] + nums[i + 1], i + 1)

只要这样，就可以保证将所有子序列，不重不漏，且按照一定大小顺序的存放进入堆
...
如果不考虑空子序列的话，那么按照上面的顺序可以枚举出所有的子序列，然而这种推到好像还看不出什么，但如果使用 mask 掩码表示数字的使用情况，其中下标为 idx 的，当 idx 置为 1 表示第 idx 位已经占用，那么显然子序列：[1]，可以表示为 001，此时再将上面的图改写一遍：
...
尽管实际元素加入堆的顺序并不是 001 -> 010 -> 011 -> 100 -> 101 -> ...(严格单调递增)，但还是可以保证小顶堆的堆顶肯定是当前枚举的最小的元素

## 如果求解的是最大和怎么办

现在需要求解第 k 大的子序列，一般来说，有两种思路

### 反向

子序列的总个数为 $2^n - 1$，按照反向思路，求解第 k 大的，就是求解第 $2^n - 1 - k$ 小的，显然，在 n 较大时，时间复杂度也是比较高的

但如果 n 比较小，且 k 比较大，这种思路也许还有点优势

### 做减法

因为全都是正数，显然最大的就是整个数组本身，可以认为：**所有其他的子序列，都是从最大的数组中减去了某些部分得到的**

首先最大的，显然就是什么都不减；而第二大的，就是从整个数组中，减去了最小的子序列...

按照这里逻辑，求解第 k 大的，其实就等价于求第 k 小的(包括)，只不过第 k 大的最大和为整个数组的和减去第 k 小的和

<hr/>

综上，显然做减法是一个更有效的做法(==特别是 k 比较小的情况==)

## 如果存在负数怎么办

如果存在负数，那么显然，最小的就应该是所有负数构成的子序列的和，**而不是数组中最小的元素了**

此时的解决方案是，**先求出所有负数的和 Sum，然后将数组中所有的负数变成正数**，并排好序，然后再按照上面的方式求解第 k 个最小的子序列和第 k 个最大的子序列，当需要返回结果的时候，**将结果和 Sum 相加后再返回**

假如存在数组: [-3,-2,-1,4,5,6]，将其取得绝对值后得到：[1,2,3,4,5,6]，对于任意一个子序列，可以通过下面的方式获取到原序列：

*   对于子序列中出现负数的绝对值，就将其去掉
*   对于子序列中不存在的负数(而在原来负数集合中出现过的负数)，将其加入子序列

比如：某一个子序列为：[1,2,5,6]，那么其实际真实的子序列应该是：[-3,5,6]

>   上面这两点描述的过程，其实就是将结果集加上 Sum 的本质

直观上，很难认同这种做法，这里也仅仅是进行说明性的推导

首先不管是否取绝对值，因为原数组和绝对值数组的长度是一样的，所以二者的子序列个数是一样的，因此可以认为原数组存在一个到绝对值数组的一一映射

既然带有负数的子序列不好求，那么而绝对值的好求，那么就先按照带绝对值的求解，然后根据映射关系得到原始序列
...
这个题，包含了负数，同时求解的是第 k 大的

那么按照上面的说法，应该求出负数的和 Sum，同时反向求解，计算出绝对值数组的总和 Total，求解绝对值数组中第 k 小的(按照小顶堆的逻辑来)，这里假设为 target

那么我们应该返回 Total - target + Sum，这里面因为 Sum 为负数，所以 Total + Sum 就等于原数组中所有正数的和，所以返回值还等于 Pos - target(其中 Pos 为所有正数的和)

```java
class Solution {
	public long kSum(int[] nums, int k) {
        // 注意这里的 sum 为所有正数的和，看不懂的，看上面的说明文字
		long sum = 0;
		for (int i = 0; i < nums.length; i++) {
			if (nums[i] >= 0) {
				sum += nums[i];
...
*   如果 dp\[i][j] = dp\[i - 1][j - 1] + 1 并且 str1[i] = str2[j]，说明此时 i 和 j 指向的位置为 comm 的一部分，将对应字符加入结果集，并且让 i 和 j 同时向前移动
    ...
    public String shortestCommonSupersequence(String str1, String str2) {
        ...
        while (i > 0 && j > 0) {
            if (dp[i][j] == dp[i - 1][j - 1] + 1 && str1.charAt(i - 1) == str2.charAt(j - 1)) {
                ...
                if (dp[i][j] == dp[i - 1][j]) builder.append(str1.charAt(--i));
                else builder.append(str2.charAt(--j));
            }
        }
        while (i > 0) builder.append(str1.charAt(--i));
        while (j > 0) builder.append(str2.charAt(--j));
...
在 c++ 里，本身就具有函数 unique() 用来去重，对于一个 vector：
...
对于这种区间的问题，显然，一个前缀和数组就能解决，但是输入数据范围来到了 $10^9$ 级别，而输入数据的个数仅为 $10^5$ 级别

如果直接开一个大小为 $10^9$ 级别的数组，那么肯定会 MLE，因此需要考虑使用离散化
...
// 注意到因为 n 和 m 的范围为 10e5，m 同时决定了左右两个区间，因此极限情况下数组大小为 3x10e5
...
int main() {
    ...
    for (unsigned int i = 0; i < adds.size(); i++) {
        pii p = adds[i];
        int idx = getIdx(p.first);
        arr[idx] += p.second;
    }
    // 计算前缀和
    for (unsigned i = 1; i <= nums.size(); i++) preSum[i] = preSum[i - 1] + arr[i];
    // 计算区间和
    for (unsigned i = 0; i < queries.size(); i++) {
        pii p = queries[i];
        int lIdx = getIdx(p.first);
        int rIdx = getIdx(p.second);
        printf("%d\n", preSum[rIdx] - preSum[lIdx - 1]);
    }
    return 0;
}
```
...
>   这个题还有一个兄弟题 [698. 划分为k个相等的子集](https://leetcode.cn/problems/partition-to-k-equal-sum-subsets/)，需要我们将数组划分为 k 份，此时 01 背包 dp 就不太好用了
...
数学性的证明就不说了，太难了，直接给出结论：**任何一个正整数都可以表示成不超过四个整数的平方之和。**

其中，对于那些满足条件 $n = 4^{k}\times(8m + 7)$ 的所有 n，均只能通过四个正整数的平方和表示
